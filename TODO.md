...
| ID | Item | Evidencia | Prioridad |
|---|---|---|---|
| §16.2.1 | **Event bus central Zustand (`sensorBus`)**. 54 hooks sensor son islas — sin correlación multi-sensor no se reducen falsos positivos. Regla: caída+inactividad+BLE desconectado=critical. **PARCIAL ✅ 2026-06-11 (cadena man-down cableada):** motor puro `src/services/sensorBus/manDownCorrelation.ts:147` (`evaluateManDownEvidence`: impacto solo→suspect; +inmovilidad→suspect con razón extra; +BLE off\|batería crítica→critical→countdown 10s→5s). Publican al bus: `src/components/emergency/FallDetectionMonitor.tsx:80` (fall), `src/hooks/useManDownDetection.ts:414` (inactivity) y `:170-211` (battery), `src/hooks/useBluetoothMesh.ts:39,120,155` (ble_proximity), `src/hooks/useGeolocationTracking.ts:103` (gps). Consumo: `useManDownDetection.ts:430-450`, flujo default intacto sin evidencia extra (no-regresión pinned). PENDIENTE: el resto de hooks sensor (HR/gas/WBGT/noise/pressure…) sigue sin publicar — las demás reglas default del bus aún no reciben señal. | `archive/IMPLEMENTATION_ROADMAP.md:645-737` + `TDA:333-335`. Cableado man-down: `docs/audits/file-ledger/PHASE5-REMEDIATION.md` (ítem A2 Capa 3). | **CRÍTICA→ALTA** |
