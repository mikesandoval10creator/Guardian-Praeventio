...
101. **Cross-Project Seismic Forge**: a member of project A creates a
...
102. **Seismic-Event Tamper / Erasure**: any user (admin included) UPDATEs a
...
103. **Schema-less Seismic Write**: a payload missing `detectedAt`/`peakG`, or
...
136. **Contractor Man-Hours Inflation (rate dilution)**: a client
...
137. **Contractor recordedBy Spoof**: a client forging `recordedBy: <someone-else>`
     ...
     path exists).
138. **Cross-project Contractor Exposure Read**: a non-member reading
...
139. **Denominator Deflation (fake >100% completion)**: a client
...
140. **Plan recordedBy Spoof**: a client forging `recordedBy: <someone-else>` on a
     safety-plan capture to misattribute the planning record — denied (server
     stamps `recordedBy` from the verified token; no client write path exists).
141. **Cross-project Plan Read**: a non-member reading
