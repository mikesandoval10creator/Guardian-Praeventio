# Política de Ventas (Freemium) - Praeventio Guard

Praeventio Guard opera bajo un modelo **Freemium**, diseñado para democratizar el acceso a la seguridad laboral básica mientras ofrece herramientas avanzadas para organizaciones de alto rendimiento.

## 1. Plan Gratuito (Free)
Acceso ilimitado a las funciones de consulta y concienciación:
- **Boletín Climático y Seguridad:** Información en tiempo real sobre condiciones del entorno.
- **Guardián Interactivo:** Consulta básica de EPP y recomendaciones de seguridad.
- **Biblioteca de Normativas:** Acceso a documentos legales y normativos estándar.
- **Botón de Emergencia:** Funcionalidad de alerta básica.
- **Red de Conocimiento (Risk Network):** Hasta 50 nodos de conocimiento personal.

## 2. Plan Premium (Suscripción)
Diseñado para Supervisores, Prevencionistas y Empresas:
- **Matriz IPERC con IA:** Generación automática de matrices de riesgo personalizadas.
- **Gestión de Proyectos Ilimitada:** Control total sobre múltiples frentes de trabajo.
- **Análisis Predictivo:** IA que predice riesgos basados en datos históricos y ambientales.
- **Risk Network Empresarial:** Nodos ilimitados y conexiones colaborativas.
- **Reportes Avanzados:** Generación de informes PDF/Excel para auditorías.
- **Soporte Prioritario:** Acceso directo a expertos en seguridad.

## 3. Filosofía de Venta
Nuestra misión es la **Prevención**. Por ello, ninguna funcionalidad crítica de vida o muerte (como protocolos de evacuación básicos o alertas de emergencia) estará nunca detrás de un muro de pago. El modelo Premium financia la investigación y el desarrollo de herramientas de IA que hacen que la gestión sea más eficiente para los profesionales.

---
*Praeventio Guard: La seguridad no es un gasto, es una inversión en vida.*
