...
### Euler-1 ola — 4 buckets + spec doc (PR #48) — **integración del trabajo de Leonhard Euler como pareja de Bernoulli**

Ola conceptualmente nueva: tras cerrar deuda técnica accionable, el usuario pidió integrar a Euler como pareja matemática de Bernoulli. Plan completo en `docs/sprints/EULER_INTEGRATION_SPEC.md` (10 fases → 3 olas Euler-1/2/3). Esta ola cubre **Fases 1, 3, 6, 10** — todas con surfaces existentes y dependencias mínimas entre sí.

- **Bucket A — Spec + Fase 1 Cartografía Topológica de Riesgos** (2 commits): `374ba99` `src/services/euler/graphConnectivity.ts` (232 LOC) + test (181 LOC, 15 tests) + barrel `index.ts`. `analyzeConnectivity` (BFS/DFS componentes + blind-spot detection) + `analyzeEulerianStructure` (oddDegreeNodes = "trampas topológicas"). Tests pinning Königsberg 4 puentes (4 nodos grado impar → no Eulerian path), triangle Eulerian circuit, linear chain, performance 1000-node <100ms. **Spec doc rescue** `4cde203` — `docs/sprints/EULER_INTEGRATION_SPEC.md` 152 LOC (10 fases × 3 olas con dependencias, criterios de aceptación, atribuciones cultas). **Wire-in en RiskNetworkExplorer.tsx deferred** — Bucket A retornó truncated; orquestador completó la spec manualmente. Origen: Euler 1736.
- **Bucket B — Fase 3 Carga Crítica P_cr Euler** (1 commit `19f829b`): `criticalLoad.ts` 145 LOC + 19 tests + `StructuralCalculator.tsx` UI section (+276 LOC con inputs E/I/L/end-conditions, output P_cr + safety factor warning <2.0) + 28 locale keys × 3 idiomas. API: `calculateCriticalLoad`, `bucklingSafetyFactor`, `rectangularInertia`, `circularSolidInertia`, `circularHollowInertia`, `EFFECTIVE_LENGTH_FACTORS` (fixed-fixed=0.5, pinned-pinned=1.0, fixed-pinned=0.699, fixed-free=2.0). Numerical pin: E=200 GPa, I=1e-6 m⁴, L=2m, pinned-pinned → P_cr ≈ 493.48 kN ±0.5%. Ratios: fixed-fixed = 4× pinned, fixed-free = 0.25× pinned. Origen: Euler 1744.
- **Bucket C — Fase 6 Método de Euler para ODE** (1 commit `f41b5d4`): `odeIntegrator.ts` ~270 LOC + 19 tests + `EmergencySimulator.tsx` (`src/components/ai/`) collapsible "Simulación de Propagación" section (5 inputs + Run + sparkline). API: `eulerStep` / `eulerIntegrate` / `eulerScalar` / `simulateFireSpread` (pre-built scenario con `phase: 'growth' | 'suppression' | 'extinguished'`). Numerical pin: y'=-y, y(0)=1, h=0.001 → y(1)≈0.36770 vs exp(-1)≈0.367879, error ~0.05%. **SLM-friendly**: pure functions, sin DOM/Worker — runnable local en Brecha B. Origen: Euler 1768.
- **Bucket D — Fase 10 Característica V-E+F=2** (1 commit `12be09d`): `polyhedronAchievements.ts` + 20 tests + `Medal3DViewer.tsx` extended con `PolyhedronMedal` + `polyhedronShape`/`progress` props (Three.js native geometries, coin medal preserved as default) + `NormativeQuiz.tsx` `userLevel` prop + live progress badge mid-quiz + results screen + fires `polyhedron:unlocked` CustomEvent + 9 locale keys × 3 langs. API: `eulerCharacteristic`, `isValidConvexPolyhedron` (V≥4 floor excludes V=1/E=0/F=1 χ=2 degenerate), `progressFromQuiz` (clamps overflow + negatives), `suggestedPolyhedron`, `PLATONIC_SOLIDS as const`. **5 sólidos platónicos wired**: tetrahedron (V=4/E=6/F=4), cube (8/12/6), octahedron (6/12/8), dodecahedron (20/30/12), icosahedron (12/30/20) — todos satisfacen V-E+F=2. Origen: Euler 1758.

Total 5 commits + master plan (4 buckets + spec rescue). **+73 tests Euler** (15 graphConnectivity + 19 criticalLoad + 19 odeIntegrator + 20 polyhedron). Suite total **2314 pass / 0 fail / 88 skipped**. Typecheck + build clean. Bundle main 305.67/310 KB (within budget). Locale cardinality verified equal across es/en/pt-BR.

**Plan Euler restante**:
- **Euler-2** (próxima): Fase 2 (caminos eulerianos en evacuación), Fase 5 (FFT identidad e^iπ+1=0), Fase 8 (Euler-Lagrange en VigilanciaScheduler)
- **Euler-3** (final): Fase 4 (ecuaciones de Euler de fluidos extends Bernoulli), Fase 7 (topología Zettelkasten), Fase 9 (ADR φ Euler en kmsEnvelope — solo doc, NO refactor)

### Decimonovena ola — 4 buckets (PR #47) — **Stryker 14/14 baseline + i18n CLOSED + análisis estructural cerrado**

- **Bucket A — `randomId` shared utility** (1 commit `b4fcae0`): `src/utils/randomId.ts` 40 LOC helper feature-detect `crypto.randomUUID` con Math.random fallback (deliberadamente NO crypto-strong para surface degraded environments, no mascarar la API faltante) · `randomId.test.ts` 8 tests covering ambas branches (modern, undefined randomUUID, undefined crypto, uniqueness, fallback shape) · refactor de **3 call sites** (`offlineQueue.ts`, `ergonomicAssessments.ts`, `iperAssessments.ts`) para usar la helper en vez de inline detect. **Fix de bug subtil**: el código pre-refactor en iper/ergo llamaba `globalThis.crypto.randomUUID()` en su "fallback" branch — habría crasheado si `crypto` undefined. La nueva `randomId()` lo arregla. **21 mutants cross-module killed esperado** (8 cada de los 3 archivos – 3 redundantes).
- **Bucket B — Stryker baseline FINAL Run #5 — 14/14 modules COMPLETE** (1 commit `fa72a2c`): los últimos 3 modules baselined — `prexor.ts` **81.71%** (82 mutants, 67 killed, 15 survived, 0 no-cov, 15s), `reba.ts` **77.74%** (310, 241, 61, 8, 1m29s), `rula.ts` **94.22%** (225, 212, 13, 0, 1m18s — *strongest module ever baselined*, eclipses iper.ts 89.36%). **Cumulative-14: 72.37%** (1273 killed / 1759 total) vs Run #4 cumulative-11 65.94% → **+6.43 pp uplift**. Removing limiters.ts (3.05% pendiente Stryker re-run en Linux CI) lifts cumulative a **~77.5%**. **NO ratchet** — limiters.ts sigue siendo el floor. Per-file thresholds remain blocked en Stryker 9.6.1 schema. McAtamney/Hignett refs documented for reba/rula context.
- **Bucket C — title= → Tooltip sweep en pages** (1 commit `b9427ab`): **9 title= sites migrated to `<Tooltip>`** + **1 semantic fix**: `Notifications.tsx` (Marcar/Eliminar todas), `Documents.tsx` (Ver/Descargar/Sin archivo), `EmergencyGenerator.tsx` (Guardar/Descargar PDF), `EmergenciaAvanzada.tsx` (Marcar seguro/peligro), `Sidebar.tsx` non-interactive badge `title=` → `role="img" aria-label="Modo Búnker (Offline)"` (badge can't receive focus, so Tooltip wrapper has no benefit; aria-label es el primitive correcto). **23 sites left alone** son: ConfirmDialog tiene NO title= attrs, component props (Modal/EmptyState/ConfirmDialog title prop son props no WCAG attrs, 10 sites), iframe title= (mandatory accessibility attr, 1 site), informational spans deferred a future passes (12 sites). aria-label preserved/added en cada migrated trigger.
- **Bucket D — offlineQueue.ts test gaps Run #3 closure** (1 commit `5528b6a`): +338 lines, **+12 tests** (7→19 totales). Targeting Run #3 baseline 60.44%: analytics emission `slm.queue.grew` 5 survivors (track shape + queue_depth_after monotonicity `[1,2,3]` + fire-and-forget fault tolerance), `:301:7 if(existing.reconciled) return;` early-return guard idempotence, `sortKeysDeep` 4 tests (top-level/insertion-order/array-order/deep-nesting), `listPending` `s.reconciled === false` filter edge cases, `deleteSession` missing-id branches. **Skipped** (out of scope, Bucket A handles): `:194:7 crypto.randomUUID` cluster. Mutation score uplift expected: 60.44% → 65-73% (short de 80% target porque randomUUID survivors están en A's scope). Stryker re-run skipped (~58s, optional).

Total 4 commits + master plan. **2241 tests pass / 0 fail / 88 skipped** (+20 vs 18va: +8 randomId + +12 offlineQueue + Tooltip migrations no test changes). Typecheck + build clean. Windows vitest worker fork transient (counts vary 2231-2241 entre runs; tercer run consistent en 2241).

**Estado deuda técnica tras 19 olas — STRYKER 14/14 BASELINE COMPLETE**:
- A11Y: **0 fail / 1 partial (monitoring) / 19 mitigated** ✅
- Analytics: **46/46 typed coverage** (43 wired + 3 surface-deferred) ✅
- i18n: **CLOSED** ✅ (2979 entries)
- STRIDE: **20 mitigated / 3 partial / 1 open** (TM-E02 ops only) ✅
- Stryker: **14/14 modules baselined (100%)** ✅, cumulative **72.37%** (~77.5% sin limiters.ts)
- CSP: nonce + strict-dynamic, `'unsafe-inline'` script-src REMOVED ✅
- Tests: 1976 → **2241** (+265, +13.4%)
- Tooltip migration: ~32 sites migrated total (RootLayout 16va + shared 17va + pages 19va)

### Decimoctava ola — 4 buckets (PR #46) — **deuda técnica accionable mayormente cerrada**

- **Bucket A — limiters.ts mutation gap closure** (1 commit `f326d7a` combined con D's docs por staging race): `+19 tests` en `src/__tests__/server/limiters.test.ts` (25 totales con los 6 IPv6 pre-existing) targeting el **3.05% mutation score crítico** identificado en Run #3. Cobertura: windowMs ArithmeticOperator (8 limiters × phase-3/4 fake-timer brackets at windowMs±1ms — pins 15*60*1000, 60*1000, 24*60*60*1000), max numeric literals (drain-to-max + max+1 → 429), keyGenerator (per-uid isolation `geminiLimiter`/`webauthnVerifyLimiter`, shared global `geminiGlobalDailyLimiter`, IP-keyed `googlePlayWebhookLimiter`, `||` chain contract), message StringLiteral/ObjectLiteral pinning per limiter, `standardHeaders:true`/`legacyHeaders:false` boolean pinning, `geminiGlobalDailyLimiter` 503 statusCode + shared key + 24h window + env fallback `'1000'`. Mutation score uplift expected: **3% → 60%+** (Stryker re-run pendiente CI Linux por Windows STATUS_STACK_BUFFER_OVERRUN crash).
- **Bucket B — webpayAdapter PCI + AUTHORIZED gate + TM-R02 closure** (1 commit `d9ed3e5`): `+22 tests` en `webpayAdapter.test.ts` (43 totales, era 21) cubriendo: PCI cardLast4 masking 4-axis (16/13/null/missing card_number, length=4 + BIN-not-leaked + exact value), **AUTHORIZED-gate quadrant matrix completo** (kills `:197:7 &&` operator + strict `=== 0`), refund mapping (toUpperCase/fallthrough/typeof), staleness-window boundary (300000 pin, strict-less-than, missing lockedAtMs). `+2 integration assertions` en `billing.test.ts`. **TM-R02 closure SOURCE-SIDE**: `billing.ts` agrega `billing.webpay-return.rejected` + `billing.webpay-return.failed` audit rows mirroring AUTHORIZED branch. **STRIDE: 20 mitigated / 3 partial / 1 open** (subió mit 19→20). Mutation score uplift expected: 58% → 70%+.
- **Bucket C — i18n FINAL CLOSURE** (1 commit `96096e5`): los últimos 3 components migrated — `ISOManagement.tsx` (45 keys UI-facing only; ISO data enums `'Vigente'/'Obsoleto'/'En revisión'` intentionally untouched para preservar Firestore equality checks, mismo patrón que ISOAudit 17va), `ExtinguisherSimulator.tsx` (17 keys), `FindTheGuardian.tsx` (24 keys). **86 unique keys × 3 locales = 258 entries**. **Cumulative locale size: 993 × 3 = 2979 translation entries** across all 18 waves. **i18n NOW CLOSED** ✅ — los 113 tsx files restantes sin `useTranslation` son test files / leaf utilities / out-of-audit-scope per spec.
- **Bucket D — Stryker baseline Run #4** (1 commit `f326d7a` combined con A's tests por staging race): 4 más modules baselined — `ergonomicAssessments.ts` **88.08%** (151 mutants, 133 killed, 17 survived, 1 no-cov), `iperAssessments.ts` **88.05%** (159, 140, 18, 1), `protocols/tmert.ts` **85.07%** (67, 57, 10, 0), `protocols/iper.ts` **89.36%** (47, 42, 5, 0). Run #4 subtotal **87.74%**. **Cumulative across 11 modules: 1142 mutants, 753 killed = 65.94%** (era 53% Run #3). **NO ratchet** — limiters.ts 3.05% (Run #3) sigue siendo el dominant constraint hasta CI re-run de A. **Top 3 actionable cross-module mutants**: (1) `crypto.randomUUID` feature-detect cluster 8 mutants en cada de ergonomicAssessments + iperAssessments + offlineQueue — **single shared utility test cierra 21 mutants across 3 files (high leverage)**, (2) Spanish recommendation StringLiteral cluster en tmert + iper, (3) `tmert.ts:84:47` boundary `hours > 24` vs `hours >= 24` (validation message says inclusive).

Total 4 commits + master plan (2 commits combined por staging race documentado). **2221 tests pass / 0 fail / 88 skipped** (+38 vs 17va: +19 limiters + +22 webpay + i18n testing). Typecheck + build clean.

**Estado deuda técnica tras 18 olas — MAYORMENTE CERRADA**:
- A11Y: **0 fail / 1 partial (monitoring) / 19 mitigated** ✅
- Analytics: **46/46 typed coverage** (43 wired + 3 surface-deferred) ✅
- i18n: **CLOSED** ✅ (2979 entries)
- STRIDE: **20 mitigated / 3 partial / 1 open** (TM-E02 ops only) ✅
- Stryker: 11/14 modules baselined (78%), cumulative **65.94%**; limiters uplift pendiente Linux CI run
- CSP: nonce + strict-dynamic, `'unsafe-inline'` script-src REMOVED ✅
- Tests: 1976 → **2221** (+245, +12.4%)

### Decimoséptima ola — 4 buckets (PR #45)

- **Bucket A — Stryker Run #2 gaps closure** (1 commit): `617ec41` cierra los NEW top 3 surviving mutants per módulo identificados en Run #2 (16va wave): verifyAuth Bearer-scheme positive-path + sepIdx cluster + StringLiteral cluster (+7 tests), orchestrator trackQueryOffline analytics seam + tryGetIdToken truthy-guard + response-shape coalescing (+10 tests), sentryInstrumentation isSentrySetupError LogicalOperator chain + sanitizeContext recursion guard + instanceof Error check (+10 tests). **Total +27 tests** across 3 modules. Mutation score uplift expected (pendiente CI cron Run #3 in same modules): verifyAuth 76→85+%, orchestrator 44→65+%, sentry 85→92+%.
- **Bucket B — Stryker baseline extension a 4 más modules** (1 commit): `918b571` `MUTATION_BASELINE.md` Run #3 con `webpayAdapter.ts` 58.26% (218 mutants, 127 killed, 69 survived, 22 no-cov, 40s), `limiters.ts` **3.05%** (131, 4, 94, 33, 45s — major test gap), `offlineQueue.ts` 60.44% (91, 55, 33, 3, 58s), `reconciliation.ts` **81.48%** (54, 44, 10, 0, 14s — best score). Run #3 subtotal 494 mutants 230 killed 46.56%. **Cumulative across 7 modules** 718 mutants 381 killed = **53.06%**. **Threshold ratchet: NO** — limiters 3.05% es nuevo floor (replazó orchestrator 43.59%). **Top 3 actionable surviving mutants cross-module**: (1) `limiters.ts:31:13/45:13` ArithmeticOperator on `windowMs: 15*60*1000` — collapsing rate-limit window sin failing test (highest-impact single fix), (2) `webpayAdapter.ts:220:11` MethodExpression on `card_number.slice(-4)` mutated → returns full PAN (PCI/PII regression risk), (3) `webpayAdapter.ts:197:7` ConditionalExpression on `responseCode === 0 && responseStatus === 'AUTHORIZED'` neutralizes AUTHORIZED gate. Windows clean: no STATUS_STACK_BUFFER_OVERRUN, Bash redirects worked.
- **Bucket C — i18n FINAL closure parcial** (1 commit): `c57e195` 6 components migrated — `AuditDetailModal.tsx` (14 keys), `ISOAudit.tsx` (26 keys, ISO_QUESTIONS data array intentionally not translated — encoded as data referenced por clause-id keys, no UI literals), `Site25DPanel.tsx` (7), `IPERCAnalysis.tsx` (23), `NormativeQuiz.tsx` (12), `ReflexBuzzer.tsx` (13). **95 unique keys × 3 locales = 285 entries**. Locale sizes 812→907 each, cardinality OK. **3 components remaining**: `ISOManagement.tsx` (614 lines — skipped por size vs time budget), `ExtinguisherSimulator.tsx`, `FindTheGuardian.tsx`. Glossary aplicado: PPE/EPI in en/pt-BR (EPP retained es), Crew/Equipe in en/pt-BR (Cuadrilla retained es), regulatory references verbatim (ISO 45001, DS 594, Ley 16.744 implicit en IPERC namespace).
- **Bucket D — title= → Tooltip migration** (1 commit): `d52ba1c` 9 components modificados con **12 title= sites migrated to `<Tooltip>` + 5 deleted como redundant** (visible button text or pre-existing aria-label) + remaining left for future wave (pages/, Sidebar non-interactive badge, iframe titles, ConfirmDialog props). Components: `SyncCenterModal.tsx`, `SyncConflictBanner.tsx`, `KnowledgeGraph.tsx`, `AsesorChat.tsx`, `WisdomCapsule.tsx`, `ModeSwitcher.tsx`, `SensoryFatigueMonitor.tsx`, `HumanBodyViewer.tsx`, `WeatherSafetyRecommendations.tsx`. Plan deviation: swapped IPERCAnalysis out (Bucket C concurrent edit), replaced con HumanBodyViewer. **WCAG state: 0 fail / 1 partial (A11Y-016 monitoring) / 19 mitigated** unchanged formally; A11Y-015 1.4.13 coverage further extended. **Bundle main 300.74 KB / 300 KB limit (+0.74 KB overage)** — atributable a sibling concurrent buckets, no a Tooltip primitive (already shipped 16va).

Total 4 commits + master plan. **2183 tests pass / 0 fail / 88 skipped** (+28 vs 16va: +27 stryker gaps + +1 indirect). Typecheck + build clean. Bundle apenas sobre limit — investigar próxima wave si se mantiene.

**Estado deuda técnica tras 17 olas**:
- A11Y: **0 fail / 1 partial (monitoring) / 19 mitigated** — sigue cerrado
- Analytics: 46/46 typed coverage (43 wired + 3 surface-deferred)
- i18n: ~3 components restantes, ~98% migrated
- STRIDE: 19 mitigated / 4 partial / 1 open (TM-E02 ops only)
- Stryker: cumulative across 7 modules **53.06%**; limiters.ts es el biggest gap (3.05% — needs urgent test additions)
- CSP: nonce + strict-dynamic, `'unsafe-inline'` script-src REMOVED
...
- **Bucket B — 5 más analytics events** (1 commit `884ff6d`): `comite.meeting.scheduled` (`ComiteParitario.tsx:83`), `comite.action_item.assigned` (`ComiteParitario.tsx:120` con `userIdHash` async fire-and-forget para hash el free-text `responsable` en `assignee_role_hash`), `comite.minutes.drafted` (`ComiteParitario.tsx:205`), `tarea.created` (`StartProcessModal.tsx:128` paired con proceso.created porque no hay client-side task-creation surface — tasks server-side via `POST /api/processes/:id/tasks`), `tarea.blocked` (`ProcessDetailModal.tsx:138` co-fires con tarea.escalated en pause con `block_reason_code: 'other'` por falta de reason selector). 3 new enums (TaskPriority, BlockReasonCode, DraftedByKind). **35/45 catalog events wired** (analytics.track call sites: 45 incluyendo type tests).
- **Bucket C — i18n closure** (1 commit `d0b4cd5` — **commit message race con Bucket B**: la commit grabbed Bucket C's staged files con Bucket B's message; verificado via `git show` que d0b4cd5 contiene SOLO los 8 i18n files): 5 dashboard components migrated — `EPPCharacter.tsx` (2 keys), `EPPRequiredWidget.tsx` (6), `DashboardQuickActions.tsx` (4), `RealTimeStatusWidget.tsx` (13), `AIInsightsModal.tsx` (7) = **32 unique keys × 3 locales = 96 entries**. Locales sizes 668→700 each, cardinality OK across all 3. **i18n NOT yet closed** — quedan ~13 components: AuditDetailModal, ISOAudit, ISOManagement, BlueprintViewer, HazmatWindOverlay, RiskNodeMarkers, Site25DPanel, HazmatStorageDesigner, StructuralCalculator, 6 gamification, IPERCAnalysis, ComplianceModal, ManDownSupervisorWidget. Several follow-up waves needed for true closure.
- **Bucket D — Server-side analytics adapter Node-compat shim** (1 commit): `75cbe0e` `serverAdapter.ts` Node-only `createServerAnalytics()` + `serverAnalytics` default singleton + `stdoutJsonSink` (one JSON line a `process.stderr`) + `serverSentryBreadcrumbSink` (cross-runtime via @sentry/core) + `createInMemoryAnalyticsQueue(max=1000)` Map-based drop-oldest+Sentry-warn on overflow + PII guard + `ANALYTICS_OPT_OUT=1` env-var opt-out · `serverAdapter.test.ts` 14 tests (track happy path, multi-sink fan-out, PII guard email/phone/rut, opt-out callback, env var, sink-fault isolation, never-throws defensive, flush empty, offline→queued→flush replay, queue overflow, idempotent clear, listPending insertion order, stdoutJsonSink writes JSON line a stderr) · `admin.ts` ambos `Sentry.addBreadcrumb('analytics.server.auth.role.*')` swapped por `serverAnalytics.track('auth.role.granted'/'revoked', ...)` con nuevo `mapToAnalyticsRole()` projecting granular Firestore role enum a 5-value analytics catalog Role enum. **Cierra el 13th wave deferral**. Browser adapter.ts UNTOUCHED — runtime separation enforced by import boundary.

Total 4 commits + master plan (commit message race en 1 de los 5 commits documentado above). **2149 tests pass / 0 fail / 88 skipped** (+40 vs 14va: +26 stryker + +14 server adapter). Typecheck + build clean.

**Ola observation tras 15 olas de multi-agent**: el patrón de commit-staging race se ha vuelto más frecuente (10va A truncated, 12va D truncated, 15va B/C colliding). Considerar próxima ola: enforce explicit `git add <list of files>` instead of relying on staged-state inheritance, OR sequence buckets that share an analytics/i18n boundary.

### Decimocuarta ola — 4 buckets (PR #42)

- **Bucket A — A11Y partials closure** (1 commit): `f842d0e` `BRAND.md` "Color tokens — accessibility constraints" subsection (~33 LOC, doc-only A11Y-002 mitigation: teal-400 not body text on white) · `HumanBodyViewer.tsx` extends `RegionPath` con `centroid`, refactor `SvgRegion` para render visual `aria-hidden` + transparent 24×24 hit-overlay (A11Y-010 — 7 of 7 regiones × 2 vistas = 14 hit overlays) · `Driving.tsx` speedometer `text-5xl sm:text-6xl md:text-7xl` + `max-w-[280px] mx-auto` para 320px reflow (A11Y-019). A11Y-015 + A11Y-016 documentados explícitamente como `partial — deferred`/`monitoring`. **WCAG estado final: 0 fail / 2 partial / 18 mitigated** (era 0/5/15 después del 13va).
- **Bucket B — 5 más analytics events** (1 commit): `ce96ea5` `suseso.form.submitted` (`AddPsychosocialModal.tsx:176` post-addNode con useRef-based time_to_submit), `proceso.created` (`StartProcessModal.tsx:115` post-fetch ok), `risk.detected.predictive` (`AlertSchedulerMount.tsx:150` per-alert en tick loop, severity bucketed por leadTimeMin), `risk.resolved` (`ManDownSupervisorWidget.tsx:70` acknowledge → resolution_kind: protocol_applied, risk_class: fall), `payment.checkout.cancelled` (`Pricing.tsx:478` cancelled-status branch additive a failed event). 4 new enums (DetectorKind 7-value, ResolutionKind, ProcesoTemplate, SusesoFormKind). Catalog extendido a **45 events** + YAML actualizado. **30/45 catalog events wired** (analytics.track call sites: 31).
- **Bucket C — i18n batch 14va** (1 commit): `4b64cce` 7 components migrados — `EventDetailsModal.tsx`, `ISOManagementFilters.tsx`, `PresentationMode.tsx`, `SafetyInspection.tsx`, `AddHygieneModal.tsx`, `ClaimForm.tsx`, `ClaimStatus.tsx`. **101 unique keys × 3 locales = 303 entries** added to common.json (sizes 567 → 668 each). Cardinality script-validated: sizes [668, 668, 668], 0-diff es-en, 0-diff es-ptBR. Nuevos namespaces: presentation, safety_inspection, hygiene, curriculum. Glossary: EPP→PPE/EPI, ODI expanded to "Duty to Inform"/"Dever de Informar", Charla→Briefing/Palestra. **Mid-session bucket reverts detectados y re-aplicados** (concurrent process restored prior tree state — agente re-hizo las 7 migrations).
- **Bucket D — Stryker first-real-run baseline** (1 commit): `5a05bdb` `MUTATION_BASELINE.md` con 3 módulos baselineados: **`verifyAuth.ts` 64.29%** (84 mutants, 54 killed, 22 survived, 8 no-cov, 47s — reproduce 8va wave smoke run) · **`orchestrator.ts` 7.69%** (78 mutants, 6 killed, 48 survived, 24 no-cov, 13s — Stryker exit 1 below break:50 by design; baja cobertura por wiring analytics reciente) · **`sentryInstrumentation.ts` 72.58%** (62 mutants, 45 killed, 15 survived, 2 no-cov, 29s). Cumulative 224 mutants, 105 killed → 46.88%, 1m29s wall-clock. **Top 3 surviving mutants**: (1) sentryInstrumentation `REDACT_KEYS` 11 StringLiteral survivors (PII redaction sin per-key assertions), (2) verifyAuth ConditionalExpression survivors en startup guard + isE2EModeEnabled (auth backdoor no exercised under prod-config), (3) orchestrator `forceOffline === true`/`forceOnline === true` decision branch (smoke coverage only). **Threshold ratchet recommendation: NO change** — orchestrator 7.69% locking in higher floor sería regression-prone; tests catch up first. Windows-specific issue: forced concurrency=1 (8va wave note); no crashes esta vez.

Total 4 commits + master plan. **2109 tests pass / 0 fail / 88 skipped** (+2 vs 13va: +2 analytics type tests). Typecheck + build clean.

### Decimotercera ola — 4 buckets (PR #41 — primer wave de "deuda técnica continua" autorizada por usuario)

- **Bucket A — A11Y fails closure** (1 commit): `8ecaf5c` cierra **A11Y-012** ComiteParitario tabs (role=tablist + role=tab + aria-selected + aria-controls + arrow-key cycle + role=tabpanel) · **A11Y-013** AddDocumentModal (role=dialog + aria-modal + aria-labelledby + Esc-to-close listener + focus return on close; focus-trap deferred porque no hay `focus-trap-react` dep) · **A11Y-009** CrisisChat menu pattern (role=menu + role=menuitem) · **verifica** A11Y-001 (lang="es"), A11Y-004 (driving-link aria-label), A11Y-007 (search input aria-label), A11Y-011 (Documents icon buttons) ya estaban fixed en 10va ola, doc updated. **Resumen final: 0 fail / 5 partial / 15 mitigated** (era 4/8/8 antes de esta ola).
- **Bucket B — i18n locale population** (1 commit): `4719c7f` puebla **132 net-new keys × 3 lenguajes = 396 entries** en `src/i18n/locales/{es,en,pt-BR}/common.json` para los componentes del 12va Bucket D (AddEventModal, DifferentialDiagnosis, DrugInteractions, VigilanciaScheduler, CloseProcessModal, StartProcessModal). 5 keys pre-existentes reutilizados (common.cancel/close, medicine.icon_alt_pill, etc.). **Cardinality verified script-validated**: sizes [567, 567, 567], es ⊃ en true, es ⊃ pt-BR true, en ⊃ es true. Glossary aplicado: Cuadrilla → Crew/Equipe, CIE-10 → ICD-10/CID-10, regulatorios chilenos verbatim (Ley 16.744, SUSESO, MINSAL, ISP Chile, PREXOR, PLANESI, TMERT, EVAST, DS 109, DS 594). 0 interpolation token mismatches.
- **Bucket C — CSP nonce + strict-dynamic** (1 commit, **`'unsafe-inline'` FULLY REMOVED de script-src**): `802c89b` `securityHeaders.ts` per-request 128-bit base64 nonce en `res.locals.cspNonce` + `script-src 'self' 'nonce-X' 'strict-dynamic' <fallback list>` · `server.ts` disabled helmet's competing CSP plugin (era overwriting nuestro nonce header), cached `dist/index.html` template at boot, swap `__CSP_NONCE__` per request · `index.html` entry script gets `nonce="__CSP_NONCE__"` placeholder · `vite.config.ts` `cspNoncePlaceholder` `transformIndexHtml` plugin (Vite owns la entry-script emission y strips arbitrary attributes; el plugin re-stamps post-build) · `csp-policy.md` reemplaza "Sprint 22+ deferred" caveat con strategy implemented + strict-dynamic rationale + dev-mode caveat + "How to add a new inline script" workflow. **+4 tests** segment-scoped regex assertion `'unsafe-inline'` ausente de script-src (pero presente en style-src para Tailwind). **Approach A** (string-replace at request time) augmented con Vite plugin necesario.
- **Bucket D — 5 más analytics events** (1 commit): `ce2b769` `auth.role.granted` (server-side `src/server/routes/admin.ts:153` deferred a Sentry breadcrumb porque adapter es browser-only — IndexedDB queue + localStorage opt-out + navigator.onLine), `auth.role.revoked` (mismo seam admin.ts:78 + 163), `knowledge.zk.node.created` (`src/services/zettelkasten/persistence/writeNode.ts:191` per-node post-success), `knowledge.zk.link.traversed` (`src/components/shared/KnowledgeGraph.tsx:711` backlink button onClick antes de pivot), `tarea.escalated` (`src/components/processes/ProcessDetailModal.tsx:120` pause-success branch). **25/44 catalog events wired** (analytics.track call sites: 24 client + 2 server breadcrumbs). Type system fully closed (compile-time guard EventName + props). +2 type-narrowing tests. Catalog + YAML extended con `tarea.escalated` row.

Total 4 commits + master plan. **2107 tests pass / 0 fail / 88 skipped** (+7 vs 12va ola: +4 securityHeaders + +2 analytics + +1 a11y net). Typecheck + build clean. PWA precache 283 entries.

**Pattern observation tras 13 olas**: el orquestador (Claude principal) ha tenido que rescatar 2 veces buckets que retornaron con messages truncados (10va Bucket A, 12va Bucket D). El patrón sigue funcionando — ambos rescates produjeron commits clean. Considerar checkpoint intermedio para agentes con scopes amplios.

### Duodécima ola — 4 buckets (PR #40)

- **Bucket A — CSP wildcard tightening + report-uri** (TM-I05 mitigated, 1 commit): `ff3b73e` `securityHeaders.ts` reemplaza `*.googleapis.com` y `*.firebaseio.com` wildcards con allow-list explícita de **11 orígenes** (firestore + identitytoolkit + securetoken + storage + firebaseinstallations + firebaseremoteconfig + fcmregistrations + generativelanguage + aiplatform + oauth2 + maps); 2 wildcards retained intencional con doc (`*.sentry.io` multi-region, `wss://*.firebaseio.com` Realtime DB) · `cspReport.ts` route 204-always con PII-stripped logging (origin+pathname only) → Sentry breadcrumb `category:'security.csp'` + captureMessage `'csp.violation'` · 8 tests cspReport + 4 tests securityHeaders updated · `server.ts` `cspReportLimiter` 50/min per-IP + 16 KB cap + dedicated `application/csp-report` MIME parser scoped to route · `csp-policy.md` allow-list table per-origin rationale + report-uri docs. **Closes TM-I05**. STRIDE: 19 mitigated / 4 partial / 1 open.
- **Bucket B — 5 más analytics events lifecycle + SLM** (1 commit): `628255c` `app.opened` (main.tsx con `detectBootKind()` discriminating cold/warm/pwa_resume via display-mode standalone + PerformanceNavigationTiming), `app.backgrounded` (visibilitychange listener + module-level session timer), `slm.queue.grew` (offlineQueue.ts post-insert con queue_depth_after), `slm.queue.reconciled` (reconciliationRunner.ts wrap performance.now con attempted/succeeded/failed/pass_duration_ms), `slm.model.downloaded` (loader.ts cache_origin: pre_packaged|cdn). **Catalog drift corrected** (boot_kind/cdn|peer|pre_packaged/pass_duration_ms/attempted-succeeded-failed/queue_depth_after-session_id/model_bytes — match catalog gold standard). **20/44 catalog events wired** (analytics.track count: 21 sites en src/). 2 type-narrowing tests + 17/17 analytics + 7/7 offlineQueue + 10/10 reconciliation + 4/4 loader pass.
- **Bucket C — A11Y batch 2** (1 commit): `92424ab` `src/index.css` global `:focus-visible` rule en `button, a, summary, [role=button], [role=switch], [role=link], [role=tab], [role=menuitem]` (teal `#4db6ac`, 2px outline, 2px offset) — closes **A11Y-006** WCAG 2.4.7+2.4.13 · driving-day `--accent-info` bumped `#4db6ac → #2e8079` para mejorar ratio 4.86:1 · `ModeSwitcher.tsx` `aria-label` verbose por slot (driving = "Activar modo conducción (día/noche automático)") preservando `aria-pressed` toggle-button pattern (más idiomático que `role=switch` para mutually-exclusive selector) — closes **A11Y-008** · `RootLayout.tsx` skip link `focus:` → `focus-visible:` Tailwind variants (teal-400 + white + rounded-md) — closes **A11Y-014** · `WCAG_findings.md` 4 rows flipped open → mitigated (incluido bonus A11Y-017 cross-referenced). Resumen 4 fail / 8 partial / 8 mitigated.
- **Bucket D — i18n batch 2 (parcial — agente truncated)** (1 commit, orquestador rescatado): `1d0eb41` `useTranslation + t('key', 'fallback')` wired en 6 components: `AddEventModal.tsx`, `DifferentialDiagnosis.tsx`, `DrugInteractions.tsx`, `VigilanciaScheduler.tsx`, `CloseProcessModal.tsx`, `StartProcessModal.tsx`. **140 unique t() keys**. **Locale entries deferred** — agente retornó con message truncado ("### 6. AddEventModal") antes de poblar `src/i18n/locales/{es,en,pt-BR}/common.json`. Runtime behavior preserved porque fallback strings son canonical Spanish; en/pt-BR muestran Spanish hasta próximo wave que pueble locales (no regresión vs antes de la migración). Patrón compatible para forward — agregar las 140 keys × 3 langs después es trabajo aislado de locales sin tocar código.

Total 4 commits + master plan. **2100 tests pass / 0 fail / 88 skipped** (+14 vs 11va ola). Typecheck + build clean. STRIDE: 19 mitigated / 4 partial / 1 open. Notable: 2 buckets seguidos (10va A, 12va D) reportaron return messages truncados — el orquestador completó manualmente en ambos casos, lo cual sugiere agregar checkpoint intermedio para agentes con scopes amplios.

### Undécima ola — 4 buckets (PR #39)

- **Bucket A — A11Y-003 + A11Y-005 fixes** (1 commit): `053c004` `--text-muted: #71717a → #52525b` (zinc-500 3.94:1 → zinc-600 7.51:1 sobre `bg-zinc-50`, pasa AA + AAA, dark-mode equivalente intacto) · `RootLayout.tsx` profile element `<div onClick>` → `<button type="button" aria-label="Ir a mi perfil">` con button-reset classes (`m-0 text-left`) preservando children/handler intactos · `WCAG_findings.md` 2 rows flipped open → mitigated. **Cierra 2 de 5 SEV2 findings** del 10va ola audit.
- **Bucket B — 4 más analytics events** (1 commit): `33835c8` `payment.checkout.started` (Pricing.tsx Webpay branch + MercadoPago branch — 2 wire-points), `payment.transaction.succeeded` (banner settled→paid), `payment.transaction.failed` (settled rejection + poll-error fallback con failure_code), `knowledge.doc.viewed` (DocsModal.tsx con dedup per-worker per-session via `useRef<Set<string>>`). **Catalog drift resolved**: plan_code/transaction_id_hash/failure_code/doc_kind/mercadopago corregidos al gold standard del `event-catalog.md`. **15/44 catalog events wired** (analytics.track count: 16 sites en src/). 2 nuevos type-narrowing tests, suite 2086 pass.
- **Bucket C — i18n batch 6 components** (1 commit): `5434a6a` `AptitudeCertificateForm.tsx` 25 keys (DS 109 form: worker/exam/dictamen/restrictions/doctor sections, exam-type + result selects), `IPERCMatrix.tsx` 20 keys (P×S labels, 5 prob + 5 sev + 4 control-effectiveness, level caption), `PlannerModal.tsx` 10 keys con plural `objectives_count` interpolation `{{completed}}/{{total}}`, `SafetyCapsules.tsx` 9 keys, `ModuleGroupsGrid.tsx` 9 keys (heading + 8 group titles via `t(\`module_groups.group_${id}\`, fallback)` lazy resolution), `QuickActions.tsx` 6 keys. **Total: 81 keys × 3 idiomas = 243 entries**. es-AR/es-MX/es-PE inherit via `fallbackLng: 'es'`. Locale parity verified by Node script (435 total keys per locale).
- **Bucket D — Express CSP + securityHeaders middleware** (1 commit): `6321210` `securityHeaders.ts` middleware con CSP completo (`default-src 'self'` + per-directive allow-lists para Sentry/Firebase/Vertex AI/Google Fonts) + 5 hardening headers (X-Content-Type-Options nosniff, Referrer-Policy strict-origin-when-cross-origin, X-Frame-Options DENY, Permissions-Policy allow-self camera/mic/geo, HSTS gated en `X-Forwarded-Proto: https` para Cloud Run) · 12 vitest tests (mock req/res, no supertest dep) · wired en server.ts ANTES de helmet ABOVE auth/routes (404s + unauth error paths carry headers) · `csp-policy.md` 1 doc con directive table, rationale, caveats, change process · `STRIDE_findings.md` TM-I05 advanced open → partial. **Defense en depth aditivo a nginx.conf 8va ola**. `'unsafe-inline'` script-src documented como deferred Sprint 22+ con nonce-based replacement.

Total 4 commits + master plan. **2086 tests pass / 0 fail / 88 skipped** (+14 vs 10va ola). Typecheck + build clean (PWA precache 283 entries +5 desde nuevos locale assets).

### Décima ola — 4 buckets (PR #38)

- **Bucket A — Analytics events batch 2** (1 commit, agente parcial rescatado por orquestador): `1d6bb5a` `types.ts` + `index.ts` 7 nuevos `EventName` (auth.user.signed_in, auth.user.signed_out, project.archived, slm.query.online, slm.query.offline, app.mode.switched, emergency.checkin.completed) con per-event interfaces + AppModeName narrowed a 3-value union matching runtime · 6 wire-points (1 import + 1 try/catch'd track call cada uno): `Login.tsx` signed_in en branch existing-user · `firebase.ts` signed_out en `logOut()` antes del signOut clear · `orchestrator.ts` slm.query.online + slm.query.offline en seam único (helpers `trackQueryOnline/Offline`, dynamic import) · `AppModeContext.tsx` app.mode.switched en setMode + auto-emergency con `modeRef` + self-transition guard · `EmergencyCheckIn.tsx` checkin.completed en `handleStatusUpdate` post-setDoc. **`project.archived` deferred** — no archive UI existe (análogo a `project.member.invited` 9na ola). **11/44 catalog events wired**. Agente lanzó pero retornó message truncado tras solo agregar types — orquestador completó wire-points manualmente.
- **Bucket B — Sentry alerts + dashboards manifest** (1 commit): `d9bdc4e` `SENTRY_ALERTS.md` narrative + 14 reglas P0-P3 con queries `level:`/`message:`/`tags.module:`/`breadcrumb.category:` que tracen a `file:line` reales · `sentry-alerts.yaml` schemaVersion 1 mirror máquina-leíble (4 P0 + 5 P1 + 3 P2 + 2 P3) · `SENTRY_DASHBOARDS.md` 3 dashboards (Overview 5 widgets · SLM Health 6 · Business Critical 5) · `dashboard-praeventio-overview.json` skeleton placeholder · `INDEX.md` cross-link a runbooks + threat model. **Top alerts**: `P0-slm-hmac-mismatch` (`reconciliation.ts:163`), `P1-slm-unsigned-legacy` count>0/1h, `P2-pii-redaction-spike` 7d baseline, `P0-webpay-error-spike` 5/5min. 1074 LOC.
- **Bucket C — Performance budgets afinados + 2 lazy-loads** (1 commit): `564f161` `lighthouserc.json` raise thresholds (perf 0.65→0.85 / a11y/best-practices/seo 0.70→0.90 / pwa 0.50→0.70 / LCP 2500→2200 / TBT 300→200) + 9 explicit metric caps nuevos (FCP 2000, CLS 0.1, Speed Index 3000, TTI 3500, unused-js 80KB, total-byte-weight 1.5MB, render-blocking 500ms) all `warn`-level hasta 4+ green CI runs · `AsesorChatLazy.tsx` deferred-mount wrapper con captured-event replay preserving event-driven open · `PredictiveAnalysis.tsx` html2canvas + jspdf movidos inside `handleDownloadPDF` · `RootLayout.tsx` wired al lazy wrapper · `PERFORMANCE.md` runbook nuevo. **Bundle delta main entry brotli: 193.5 KB → 157.96 KB = -35.5 KB (-16.4%)**. New chunks: `AsesorChat` 3.79 KB br + react-markdown graph 29.96 KB br, neither during auth flow.
- **Bucket D — WCAG 2.2 AA audit + 5 micro-fixes** (1 commit): `2430b12` `A11Y_AUDIT.md` (methodology, 4-mode contrast tables normal-light/normal-dark/driving/emergency, surface coverage 12 pages, prioritized backlog, living-doc policy) · `WCAG_findings.md` **20 findings** con file:line (5 fail / 12 partial / 2 mitigated / 1 pass-with-note · 0 SEV1 después de fix · 6 SEV2 · 14 SEV3) · `checklist-WCAG-2.2-AA.md` matriz **52 criteria** PASS/PARTIAL/FAIL/N/A · `tests/e2e/accessibility.spec.ts` extendido con `/login` scan + shared `runAxe` helper + `wcag22aa` tag + TODO auth-gated routes. **5 micro-fixes**: `index.html lang="en"→"es"` (A11Y-001 critical), `RootLayout.tsx` search input + safe-driving Link aria-label + icon aria-hidden, `Documents.tsx` View button + MoreVertical aria-label/haspopup/expanded. **Top 3 críticos abiertos**: A11Y-003 `--text-muted:#71717a` ratio 3.94:1 cascadea a Login/Dashboard/Documents (mitigation: zinc-600 = 7.51:1), A11Y-005 RootLayout profile button es `<div onClick>` no focusable, A11Y-008 driving mode toggle.

Total 4 commits + master plan. **2072 tests pass / 0 fail / 88 skipped** (consistent con 9na ola). Typecheck + build clean. Bundle main brotli -16.4%.

### Novena ola — 4 buckets (PR #37)

- **Bucket A — PII redactor Vertex AI** (TM-I03 mitigated, 1 commit): `b76cdd5` `piiRedactor.ts` con regex puro (RUT chileno con/sin puntos, email, teléfono CL +56 9 XXXX XXXX, credit card heurística, API keys `sk-/AIza/ya29.`), retorna `{redacted, count, categories}` idempotente · 15 tests vitest (categorías individuales, multi, idempotencia, unicode, dedupe) · `geminiBackend.ts` wired en 9 sites (`analyzeFastCheck`, `analyzeRiskWithAI`, `processDocumentToNodes`, `analyzeRootCauses`, `queryBCN`, `getChatResponse` incl. history, `investigateIncidentWithAI`, `analyzeFeedPostForRiskNetwork`, `extractAcademicSummary`) vía `redactPromptForVertex(prompt, action)` helper único · Sentry breadcrumb `category: 'pii.redaction'` cuando count > 0 (sin valor redactado, solo count + categories). **Closes TM-I03 — Ley 21.719 art. 50**.
- **Bucket B — IndexedDB queue HMAC** (TM-T03 mitigated, 1 commit): `305f829` `hmac.ts` con Web Crypto API (HMAC-SHA256, key 256-bit en `sessionStorage` base64 raw bytes, in-memory fallback para Node tests, `__resetSessionKeyForTesting`) · 12 tests hmac (round-trip, tampered payload/tag, malformed, determinism, unicode, long) · `offlineQueue.ts` `QueuedSession.hmac?: string` opcional + `canonicalForHmac()` recursive key sort + `deleteSession(id)` · `reconciliation.ts` verify antes de writeNodes; mismatch → `Sentry.captureMessage('slm.queue.hmac_mismatch', warning) + deleteSession`; legacy (sin hmac) → breadcrumb `slm.queue.unsigned_legacy` passthrough; wrap entero en `withSentryScope('zettelkasten', { action: 'reconcile' })` · 4 tests reconciliation (tampered drop+warn, legacy passthrough). **Closes TM-T03 — defense en passive IndexedDB tampering**. Migration: legacy entries pasan una sola vez con breadcrumb; flip a drop en Sprint 22 cuando monitoring confirme cero legacy.
- **Bucket C — Stryker consolidación** (1 commit): `e81e620` mergea `stryker.conf.json` (R20 ergonomics + protocols: 7 modules — `safety/ergonomicAssessments`, `safety/iperAssessments`, `protocols/{tmert,iper,prexor}`, `ergonomics/{reba,rula}`) en `stryker.config.json` único de octava ola (security + business: 7 modules) → **14 modules totales con un solo source of truth**. Threshold conservador `{high:80, low:60, break:50}` (lower de los dos para no regresar). `timeoutMS: 60000` (higher), `excludedMutations: ['ArrayDeclaration']` preservado (canonical RULA/REBA tables). `stryker.conf.json` deleted via `git rm`. `package.json` +3 scripts (`test:mutation:ergonomics`, `test:mutation:protocols`, `test:mutation:safety`). Smoke run `verifyAuth.ts`: **64.29% mutation score** confirma plumbing OK.
- **Bucket D — Analytics adapter + 4/5 activation events** (1 commit): `d59cddb` 6 archivos nuevos en `src/services/analytics/`: `types.ts` 213 LOC (EventName union 5 events + per-event prop interfaces + Sink contract) · `queue.ts` 175 LOC (IndexedDB store separada `gp_analytics_queue` MAX 1000 con drop-oldest+Sentry-warn) · `sinks.ts` 95 LOC (noop / console-dev / sentry-breadcrumb-prod) · `adapter.ts` 320 LOC (`AnalyticsAdapter` con PII guard hard-drop si `email/phone/rut/name/address` aparecen, opt-out via `localStorage.analytics_opt_out`, common-props auto-fill, sha256 `userIdHash` salted con `VITE_ANALYTICS_SALT`) · `index.ts` barrel + singleton `analytics` con `[consoleSink, sentryBreadcrumbSink]` · `adapter.test.ts` 13 tests. **4 wire-points landed**: `auth.user.signed_up` en `Login.tsx:64`, `project.created` en `ProjectContext.tsx:143`, `risk.reported.manual` en `AddFindingModal.tsx:156`, `tarea.completed` en `ProcessDetailModal.tsx:262`. **5to deferred** (`project.member.invited`): no client-side invitation UI hoy (solo `POST /api/projects/:id/invite` server-only). Mappers Spanish→catalog enums (`mapIndustryToCode`, `mapCategoryToRiskClass`, `mapSeverityLabel`).

Total 4 commits + master plan. **2072 tests pass / 0 fail / 88 skipped** (+44 vs 8va ola: 15 piiRedactor + 12 hmac + 4 reconciliation + 13 analytics). Typecheck + build clean. STRIDE findings: 18 mitigated / 4 partial / 2 open (TM-E02 Cloud Run SA scope sigue abierto; TM-I03 + TM-T03 ahora cerrados).

### Octava ola — 4 buckets (PR #36)

- **Bucket A — Cloud Build CI alternativa** (Fase 3 alt path, 1 commit): `b5163c3` `cloudbuild.yaml` (substituciones `_LOCATION=southamerica-west1`, machineType E2_HIGHCPU_8, timeout 1200s, build-and-push only — deploy manual) · `Dockerfile.api` multi-stage (node:20-alpine, USER node, healthcheck, ENTRYPOINT `tsx server.ts` por realidad runtime sin compilado JS) · `Dockerfile.frontend` multi-stage (build → nginx:alpine, USER nginx) · `nginx.conf` SPA fallback + headers conservadores (no rompe Sentry/Firebase) · `.gcloudignore` + `.dockerignore` actualizados · `CLOUD_BUILD_RUNBOOK.md` con prereqs IAM, deploy/rollback manual, costo. **GH Actions deploy.yml intacto — Cloud Build es aditivo.** 717 LOC.
- **Bucket B — STRIDE threat model** (Fase 8 inicial, 1 commit): `5fd700c` `THREAT_MODEL.md` (9 secciones: scope, system overview, trust boundaries, assets, adversary models, methodology, walkthrough STRIDE 6 categorías, prioritized backlog, living-doc policy) · `STRIDE_findings.md` tabla 24 hallazgos con `file:line` reales (16 mitigated / 5 partial / 3 open · 2 critical, 16 medium, 6 low · 5 S, 4 T, 2 R, 6 I, 4 D, 4 E) · `data-flow-diagram.md` Mermaid `flowchart LR` con 5 trust-boundary subgraphs (browser, mobile, GCP, external GCP, third-party). **Top 3 open**: TM-I03 PII redaction Vertex AI (`gemini.ts:238-251`) · TM-T03 IndexedDB queue HMAC (`offlineQueue.ts:36-105`) · TM-E02 Cloud Run SA scope ops audit. 482 LOC.
- **Bucket C — Stryker mutation testing config** (CC-4, 1 commit): `f8a6a8d` `stryker.config.json` (vitest runner, perTest coverage, concurrency 4, thresholds high=80/low=60/break=50, mutate 7 archivos críticos: verifyAuth, limiters, SLM trio, sentryInstrumentation, billing/webpayAdapter) · `package.json` +3 scripts (`test:mutation`, `test:mutation:auth`, `test:mutation:slm`) pin a `stryker.config.json` para no chocar con `stryker.conf.json` legacy de R20 ergonomics · `.gitignore` `.stryker-tmp/` + `reports/mutation/` · `MUTATION_TESTING.md` 108 LOC con rationale, run guide, threshold policy, deferred CI integration. **Dry-run en verifyAuth con config legacy: 84 mutants, 64.29% score** (plumbing OK; Linux CI rendería mejor que Windows host). 141 LOC.
- **Bucket D — Tracking design plan** (CC-1 follow-up, 1 commit): `eebcdbd` `TRACKING_PLAN.md` 12 secciones (scope, goals, audit chain, principles, taxonomy, funnels, governance, privacy, backend recommendation, open questions, implementation handoff, changelog) · `event-catalog.md` **44 eventos** en 12 surfaces (lifecycle 22 / engagement 10 / safety_critical 9 / commerce 3) · `property-glossary.md` **102 propiedades** (11 common + 91 event-specific, PII risk row-by-row, enums enumerated) · `.telemetry/proposed-events.yaml` schemaVersion 1, planVersion 1.0.0 manifest máquina-leíble cardinality 44↔44 catalog↔YAML. **Recomendación: PostHog OSS self-hosted southamerica-west1** (Ley 21.719 residencia datos), decisión deferida con fit-score table. Anclajes: hashed `userId` (sha256+salt rotativo), `commune_code`-only geo en emergency, mode-aware sampling (driving 50%, emergency 100%). 1040 LOC.

Total 4 commits + master plan. **2028 tests pass / 0 fail / 88 skipped**. Typecheck + build clean. Conflict flagged: `stryker.conf.json` legacy R20 + `stryker.config.json` octava ola coexistiendo — consolidar en próxima ola.
