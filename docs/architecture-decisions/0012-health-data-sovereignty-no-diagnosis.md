...
6. ✅ **Bucket XX (DONE)** — Code review automation: pre-commit hook que
   rechaza palabras prohibidas + falta de `<MedicalDisclaimer/>` en
   archivos del módulo médico. Implementado en
   [`scripts/precommit-medical-guard.cjs`](../../scripts/precommit-medical-guard.cjs),
   wired vía [`.husky/pre-commit`](../../.husky/pre-commit) y replicado
   en CI como job `adr-0012-enforcement` en
   [`.github/workflows/ci.yml`](../../.github/workflows/ci.yml).
   Tests stdlib `node:test` en
   [`src/__tests__/scripts/medicalGuard.test.cjs`](../../src/__tests__/scripts/medicalGuard.test.cjs).
   Override de emergencia: `git commit --no-verify` con justificación
   en el commit body — el job CI vuelve a correrlo, así que solo difiere
   el bloqueo, no lo elimina.
