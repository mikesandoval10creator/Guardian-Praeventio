...
- [~] **F2 `parseGeminiJson` + codemod (P11)** — parser tipado + fallback 502; aplicar a los
  `*Backend.ts` (medicine/psychosocial/suseso/legal/safetyEngine/shift/prediction/network). (vitest)
  → ✅ PARCIAL #658/#769/#772/#784: el parser canónico es `parseGeminiJson` en `src/services/gemini/_shared.ts:55`
  (NO el filename asumido `parseGeminiJson.ts`); #772 migró 11 parses genuinos; #769 legal; #784 alineó 4 tests stale.
  **PENDIENTE (5 callsites con `JSON.parse(response.text || '{}'/'[]')` aún crudos, empty-coercion):**
  `geminiBackend.ts:232,707,1005,1208` + `gemini/embeddings.ts:112`.
...
- [ ] 🟡 🔵 BLOCKED: `AlertScheduler` `probes={[]}` (`RootLayout.tsx:467`) → **cablear** probes reales (Bernoulli predictivo). (comp) — sin feed predictivo real montado; necesita fuente Bernoulli antes de cablear (no fabricar probes).
...
- [ ] 🔵 BLOCKED: `FirstResponderDispatchPanel` huérfano → **falta fuente de datos real (decisión de producto).** El motor `firstResponderMap.ts` + ruta + componente están completos y testeados, pero `buildDispatchPlan`/`analyzeCoverage` exigen `Responder[]` con la taxonomía de primer-respondedor (`paramedic`/`rescue_specialist`/`sifCertified`) + `currentPosition` + `availability` en vivo. NO existe ese feed en el repo: `BrigadeMember` (`useEmergencyBrigade`) no tiene posición, ni `availability`, ni esos roles (es roster de brigada, dominio distinto). Cablearlo a brigada **fabricaría** posiciones (modo dispatch) o daría una cobertura siempre-crítica distorsionada (modo coverage) → viola "hacer real, no fabricar". Requiere primero un **subsistema de presencia de respondedores** (geolocalización on-device + disponibilidad, sensible a privacidad Ley 19.628) — decisión de producto/arquitectura, no auto-cableable. NO montar con datos ficticios.
...
- [~] 🟡 `VigilanciaScheduler` DEMO_EXAMS (`Medicine.tsx:140`) → **cablear a exámenes reales**; `Hygiene.tsx` métricas hardcoded → **cablear a métricas reales**. (comp) → ✅ PARCIAL #763 (`VigilanciaScheduler.tsx:60` filtra `LegalObligation.kind==='medical_exam'`, DEMO_EXAMS eliminado, estado vacío honesto). **PENDIENTE:** `Hygiene.tsx:138` aún pinta un bar-chart con valores hardcoded `[40,65,45,80,…]`.
...
- [~] 🔴 `cphs_meetings:1175` append-only no preserva prefijo del array de firmas **✅ PARCIAL (post-firma)** (la rama APPEND, Caso B, validaba sólo que el array creciera en 1 sin asegurar el prefijo → un miembro podía reescribir/forjar co-firmas previas; ahora Caso B exige array nuevo == array viejo + 1 elemento appendeado (concat-equality), firmas previas inmutables; +7 rules-tests RED→GREEN + Dirty-Dozen 53-55; PR #774). **PENDIENTE (primera firma):** la rama Caso A (`resource.data.signatures.size() == 0`, `firestore.rules:1466-1471`) permite pasar de `[]` a CUALQUIER array `signatures` en un solo update validando sólo `committeeId`/`scheduledAt` → un miembro puede sembrar N firmas WebAuthn forjadas antes de la primera firma legítima, evadiendo el append-one para el lote inicial. Falta gate de primera-firma + test. (rules)
