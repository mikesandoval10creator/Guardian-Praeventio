...
**CPHS→server (próximo bloque, scopeado):** `ComiteParitario.tsx:76/113/149` escribe `comite_actas` (acta legal) directo desde cliente con `addDoc`/`updateDoc`, SIN audit server → mover a ruta server (create acta / add acuerdo / update estado) con `verifyAuth`+`assertProjectMember`+`auditServerEvent` (CLAUDE.md #3 identidad-desde-token). `cphsMinute.ts` server solo tiene GET `/draft-minute` (autogen). Regla `comite_actas` → write server-only.
