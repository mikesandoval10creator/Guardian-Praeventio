...
| evac cross-device join + stale-resume | (en review) | un 2º supervisor que da "iniciar" con drill activo se UNE al conteo en curso (409→adopt drillId) en vez de error crudo; retomar un drill inexistente muestra aviso en vez de pantalla de inicio silenciosa | `hooks/useEvacuationHeadcount.ts` EvacuationAlreadyActiveError + `components/evacuation/EvacuationDashboard.tsx` adopt-on-409 + staleResume |
...
**Follow-ups OLA 1 (anotados):** superficie UI "tu alerta SOS no salió" (completa #889, toca RootLayout); join DEA/refugio más cercano en escalación (completa #890); ~~mandown-cron~~ (✅ #894 runManDownEscalation); ~~stop-work resume auditado+firmado~~ (✅ #901); ~~lone-worker start audit~~ (✅ #902); ~~DECLARE/precondition stop-work + audit→Sentry~~ (en review, `task_a070c074`); CI "Bundle size budget" flakea con HF 429 en el build redundante del size-limit-action (`task_806d1814`); sos-e2e des-fixme (con #887 ya en main, falta seed phone + assert tel: + corrida full-stack).
