...
| SIF precursors page | #899 ✅ | montó el SIFAlert huérfano en `/sif` + ruta auditada notify-mandante (marca interna, NO push a organismo) | `pages/SifPrecursors.tsx` + `server/routes/sif.ts` `POST /:projectId/sif/:id/notify-mandante` + `useSif.ts` recordSifMandanteNotification |
| evacuación consolidación | #900 ✅ | 2 tableros → 1 board live server-backed (el IndexedDB single-device era invisible al resto en evac real); roster-gate start + resume-by-query + 409 doble-inicio | `pages/EvacuationDashboard.tsx` (contenedor) + `components/evacuation/EvacuationDashboard.tsx` (board) + `evacuationHeadcount.ts` `/start` 409 + `evacuationFirestoreAdapter.ts` listActive |
| stop-work resume firmado | #901 ✅ | cerró el hoyo authz (resumedByRole del cliente → token + isApproverRole 403) + audit + modal firmado huérfano montado (reemplaza auto-resume client con rol hardcoded) | `server/routes/stoppage.ts` `/resume` server-authoritative (txn+audit+409) + `stoppageEngine.ts` StoppageResumption + `useStoppage.ts` resumeStoppage real + `pages/StoppageMonitor.tsx` modal + fail-loud feed |
| lone-worker start audit | #902 ✅ | auditó el INICIO de sesión solitario (única acción del lifecycle sin audit; check-in/end ya auditados); workerUid del token, id server-minted (mató Math.random). El review cazó 2 rutas de inicio → arregló LoneWorker.tsx + LoneWorkerMonitor.tsx | `server/routes/loneWorker.ts` `POST /:projectId/lone-worker/start-session` + `loneWorkerService.ts` startLoneWorkerSession + `useLoneWorker.ts` startLoneWorkerSessionApi + `pages/LoneWorker.tsx` + `pages/LoneWorkerMonitor.tsx` |
| stop-work declare/precondition | #903 ✅ | sibling de #901: `/declare` deriva declaredByRole del TOKEN (no body) + id server-minted + audit; mark-precondition auditado (verifierUid del token); `auditLog.ts` reenvía a Sentry (#14, todas las rutas); StoppageMonitor rutea declare+precondition por server + gate de categoría por rol | `server/routes/stoppage.ts` declare+markPrecondition + `server/middleware/auditLog.ts` Sentry + `hooks/useStoppage.ts` + `pages/StoppageMonitor.tsx` |
| CI flake bundle-size | #904 (CI) | `size` job de perf.yml: SLM_PREPACKAGE_SKIP a env de JOB → el build interno de size-limit-action ya no fetchea Qwen de HF (429 dejaba de tumbar el gate) | `.github/workflows/perf.yml` |
| evac cross-device join + stale-resume | #905 ✅ | un 2º supervisor que da "iniciar" con drill activo se UNE al conteo en curso (409→adopt drillId) en vez de error crudo; retomar un drill inexistente muestra aviso en vez de pantalla de inicio silenciosa | `hooks/useEvacuationHeadcount.ts` EvacuationAlreadyActiveError + `components/evacuation/EvacuationDashboard.tsx` adopt-on-409 + staleResume |

**Correcciones verificadas al scoping (el digest se equivocó):** (1) el master-gate `match /{subCollection=**}/{docId}` (firestore.rules:359) da lectura recursiva a `projects/{pid}/**` → las "colecciones sin regla" bajo `projects/` (lone_worker escalations, etc.) YA son legibles; solo `tenants/{tid}/...` necesita regla. (2) **RESUELTO #901:** StoppageMonitor persistía el resume vía `updateStoppageStatus` bypasando la ruta server auditada + rol hardcoded `'supervisor'` + modal firmado huérfano + auto-resume client al cumplir preconditions. Ahora `/resume` es server-authoritative (rol del token, isApproverRole 403, signatureAttested obligatorio, txn + audit + 409), el modal firmado se monta, y el auto-resume client se eliminó.

**Follow-ups OLA 1 (anotados):** superficie UI "tu alerta SOS no salió" (completa #889, toca RootLayout); join DEA/refugio más cercano en escalación (completa #890); ~~mandown-cron~~ (✅ #894); ~~stop-work resume auditado+firmado~~ (✅ #901); ~~lone-worker start audit~~ (✅ #902); ~~DECLARE/precondition stop-work + audit→Sentry~~ (✅ #903); ~~CI Bundle-size HF429 flake~~ (✅ #904); ~~evac cross-device~~ (✅ #905); sos-e2e des-fixme (con #887 ya en main, falta seed phone + assert tel: + corrida full-stack) — ÚNICO item OLA 1 restante.

## OLA 2 — blindaje (iniciada 2026-06-14)

**Hallazgo de scoping (Explore very-thorough): el "~50 colecciones sin regla" del plan es MAYORMENTE falso positivo.** El master-gate recursivo `projects/{pid}/{=**}` (firestore.rules:359) cubre todo `projects/{pid}/**`; las reglas explícitas cubren zone_violations/root_cause_analyses/lone_worker_events/legal_obligations/driving_reports/vector_store. **GAPS REALES = 4, todas server-written (Admin SDK bypassa rules en WRITE) → LATENTES** (no rompen prod hoy; un READ de cliente futuro caería en default-deny). Causa: el bloque `tenants/{tid}/projects/{pid}` (firestore.rules) no tiene catch-all recursivo (solo site_geometry+evacuations explícitos).

| Bloque | PR | Qué cierra | file:line |
| --- | --- | --- | --- |
| server-only tenant stores rules | (en review) | reglas explícitas read=member + create/update/delete=false (server-only) para `positive_observations`/`photo_evidence`/`splat_captures` bajo tenants/{tid}/projects/{pid} (3 de los 4 gaps reales) + 15 rules-tests (emulador, no skip) + Dirty Dozen #115-117 | `firestore.rules` (bloque tras evacuations) + `src/rules-tests/tenantServerWriteStores.rules.test.ts` + `security_spec.md` |

**Pendiente OLA 2:** 4º gap `predictive_alert_acks` (top-level, server-write, LOW); retención PII (reaper cron — `privacyRetention.ts` hoy solo advisory `decide-retention`, NO borra); CPHS server (ver abajo); F4 WebAuthn verify. **OJO: el "rules gap" resultó deuda de convención (#4)+future-proof, NO falla activa de vida — re-priorizar OLA 2 hacia valor-activo.**

**✅ Avance sesión 2026-06-16 (autopilot OLA 2):**
- **RAG tenant-scope (privacidad) ✅ #930** — read-path del RAG legal scopeado a law-only + 2 HIGH de correctitud (ver B14 arriba). Chip durable `task_ea3ba907` para migración a subcolecciones por-tenant.
- **tier-gating server-side ✅ #932 (report-only)** — `requireTier` montado en insights/multiProject/maturity/multiRoleSummary (platino), per-route tras verifyAuth (NUNCA por mount: `/api/sprint-k` lo comparte el historial-propio GRATIS). `tierRouteTable.ts` = política central + invariante ADR 0021 keyword-based. Flip `TIER_GATE_ENFORCE=true` (redeploy) para enforce. Sigue siendo el "mayor valor activo": faltan ~13 rutas premium más por mapear (SSO/Drive titanio ya parcial, B2D API platino).
- **portable-history cross-tenant ✅ #931** (Ley 19.628) — `incidents` global scopeado a projectId + fallback `users/{uid}` solo owner-self (un admin leía PII de otro tenant) + dedup. Review adversarial halló los últimos 2.
- **IAP planId ✅ #929** — todas las compras IAP se mis-granteaban `comite` (`normalizeSubscriptionPlanId(SKU)`=null) → `planFromIapProductId`. Follow-up LOW pendiente: observabilidad de billing (cycle-en-audit + warn-on-default + IAP unknown-SKU fail-loud).
- **billing observabilidad + cycle ✅ #934** — `resolveInvoiceCycle(invoiceData):{cycle,source}` (subscriptionPlan.ts); cycle propagado a audit rows en webpay/khipu/invoices/googleplay/mercadoPagoIpn/appleSsn/subscription; googleplay `/verify` fail-loud 400 en SKU desconocido.
- **CPHS→server ✅ #935** — 3 endpoints writer auditados (`cphsMinute.ts` create acta / add-acuerdo / PATCH-estado con `runTransaction` #19) + `verifyAuth`+`assertProjectMember`+`auditServerEvent`; `comite_actas` write server-only. Cierra el "próximo bloque" CPHS.
- **ADR 0024 retención ✅ #936** — decisión fundador: Praeventio NO borra datos de prevención (DS 594 7yr); única supresión = anonimización ARCO del trabajador. Cierra el ítem "reaper cron" por NO-construir (sin reaper, sin delete empresa-facing). Ver memoria `project_pii_retention_findings_2026-06-16`.
- **F4 WebAuthn aptitud ✅ #937** — firmador de certificado de aptitud DS 109 pasó de 503 a verificación REAL vía el verificador canónico (`verifyWebAuthnAssertion`), `signedAt` server-stamped, origin fail-loud en prod. Seguridad-crítica, review obligatorio aplicado.
- **pagos fail-closed pre-clave ✅ #938** — Khipu webhook fail-OPEN (verificaba HMAC contra el secret sandbox público si `KHIPU_SECRET` ausente → factura 'paid' forjable) → 503; MP_ENV='prod' cobraba CERO (sandbox) → acepta 'prod'|'production'; Webpay return self-guard. Auditoría "código real ANTES de claves".
- **honest-data / silent-failure ✅ #939** — P10 datos-inventados: ClimateRoutes pines falsos `Niebla`/`Nevazón` removidos (cierra el "teatro" del audit de mapas) + WeatherBulletin ignoraba el sentinel `{unavailable:true}` + SloErrorBudget curva sintética → null. Review halló 3 P1 donde la fabricación sobrevivía por otra capa (`uv:5` mockeado en orchestrator derrotaba el label `(est.)`; badge verde junto a "Datos no disponibles"). Ver `project_honest_data_2026-06-16`.
...
- [~] 🔴 `networkBackend.ts:41,77` RAG-poisoning + cross-tenant → F3 + scope (`vector_store` por tenant). (super)
  → ✅ **WRITE-path ya blindado** (B14: `syncNodeToNetwork` usa projectId canónico + `assertProjectMember`, cierra write-poisoning).
  → ✅ **READ-path scope (#930)**: el RAG **legal** (`safeNormativeQuery.ts`) hacía `vector_store.findNearest` SIN filtro → podía devolver texto de nodos privados de OTRO tenant como "contexto legal". Ahora over-fetch + post-filtro law-only (`lawId!==undefined && nodeId===undefined`). Además 2 HIGH de correctitud hallados en review y arreglados en el mismo bloque: `distanceResultField` (sin él, el SDK no popula `data.distance` → en prod TODO score=0 → `no_verified_match` permanente, enmascarado por el mock) + over-fetch `max(k*40,150)`+clamp topK anti-starvation. Test del leak en `safeNormativeQuery.test.ts` + mock actualizado en `ragService.test.ts`.
  → **PENDIENTE `[~]` (chip `task_ea3ba907`):** fix DURABLE — mover node-vectors a subcolecciones por-tenant (patrón `incident_vectors/{tid}`), dejando `vector_store` law-only → el post-filtro pasa a defensa-en-profundidad y la starvation es estructuralmente imposible. Requiere índice vectorial + reglas + ≥5 rules-tests + job de backfill.
