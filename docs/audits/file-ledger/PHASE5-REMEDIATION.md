...
**Follow-ups OLA 1 (anotados):** superficie UI "tu alerta SOS no salió" (completa #889, toca RootLayout); join DEA/refugio más cercano en escalación (completa #890); ~~mandown-cron~~ (✅ #894 runManDownEscalation); ~~stop-work resume auditado+firmado~~ (✅ #901); **DECLARE/precondition stop-work aún client-side con rol hardcoded** (un worker puede fingir declare de approver → `task_a070c074`); audit→Sentry en `auditLog.ts` (helper compartido, afecta todas las rutas); sos-e2e des-fixme (con #887 ya en main, falta seed phone + assert tel: + corrida full-stack).
