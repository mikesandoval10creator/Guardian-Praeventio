...
- **F7 · Evidencia inmutable (2026-07-02):** los documentos NO se borran desde el cliente — un delete es primitiva de destrucción de evidencia legal (DS 44 / Ley 16.744). Rules `delete: if false` en Storage (`projects/**`, `blueprints/**`, `documents/{workerId}/**`; `reconstructions` queda fuera: asset 3D de trabajo) y Firestore (`project_documents` + update solo-`archived`, `projects/*/workers/*/documents`, `projects/*/documents`, `workers/*/documents`, `comite_actas`). UI reconvertida Eliminar→Archivar (`archived: true`, hide-only) en DocsModal y ProjectDocuments. Remoción física gobernada = solo server-side (Admin SDK + audit_logs). Vault personal `workers/{uid}` (copias propias del trabajador) fuera del alcance. **Follow-up pendiente:** sweep F7 sobre el resto de colecciones legal-trail con `delete: isAdmin()||isSupervisor()` (findings, clinical_alerts, inspections, …) — decidir colección por colección con el fundador.
