...
- **4 PRs abiertos problemáticos** (cascarón/dato-falso): #1051 useAdoption (`hasPaidPlan:true`/`activeProjects:1` fabricados → usar SubscriptionContext/ProjectContext), #1059 useRoiScenario (fetch descartado, no renderiza), #1055 useVendorOnboarding (`compliance:[]`/`requirements:[]`), #1049 useProjectComparator (montado sin `snapshots` → usar `useMultiProject`). #1036 SupervisorBriefingCard (arrays vacíos).
