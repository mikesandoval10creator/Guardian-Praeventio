// Praeventio Guard â€” Round 17 R3.
//
...
//   â€¢ POST /api/push/register-token
//
// Behavior contract (covered by I3 supertest harness â€” see
// src/__tests__/server/push.test.ts):
//   â€¢ 401 when no Bearer / malformed (handled by verifyAuth)
//   â€¢ 400 invalid token (empty, non-string, >512 chars)
//   â€¢ 400 invalid platform (must be 'ios' | 'android' | 'web')
//   â€¢ 200 + arrayUnion write to users/{uid}.fcmTokens + audit_logs row
//   â€¢ 500 on Firestore failure
//
...
//   1) audit_logs is append-only by firestore.rules â€” leaking a token there
//      gives anyone with read-audit privileges the ability to push to that
//      device until the token rotates.
//   2) Compliance audits (Ley 16.744) routinely export audit trails â€” the
...
router.post('/register-token', verifyAuth, async (req, res) => {
  ...
  try {
    ...
    // Audit trail â€” see audit_logs schema notes in server.ts. We deliberately
