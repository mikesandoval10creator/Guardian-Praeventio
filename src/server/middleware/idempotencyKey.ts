...
export function idempotencyKey(opts: IdempotencyKeyOptions = {}) {
  ...
  return async function idempotencyKeyMiddleware(
    ...
    const user = (req as any).user as { uid?: string; tenantId?: string } | undefined;
    if (!user || !user.uid) {
    ...
    // ── Step 1: read existing cache entry ─────────────────────────────
    ...
    // ── Step 3: cache miss → wrap res.json/.send to capture response ──
