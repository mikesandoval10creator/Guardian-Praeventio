...
//
// SPI (Safety Performance Index) — close the default-deny gap on
// `safety_plan_periods`.
//
// Path: safety_plan_periods/{projectId_YYYY-MM} — the captured PLANNED counts
// (inspections / daily talks / trainings) for a project+period: the
// DENOMINATORS for the leading indicators of the SPI (executed ÷ planned).
// WRITTEN ONLY by the Admin SDK from POST /api/sprint-k/:projectId/
// safety-performance/safety-plan, which role-gates (admin/gerente/
// prevencionista-tier) and stamps `recordedBy`/`recordedAt` from the verified
// token. READ is open to any member of the doc's project (the SPI dashboard
// renders the plan-vs-executed ratios).
//
// A client write is a spoof/integrity vector: forge `recordedBy`, or DEFLATE a
// planned count to fabricate a >100% completion ratio (make the crew look
// compliant by shrinking the denominator). This suite pins member-read-allow +
// total client write denial. (CLAUDE.md #4 — ≥5 rules-tests; Dirty Dozen in
// security_spec.md.)
//
...
const PID = 'proj-spi-1';
const MEMBER = 'member-spi-1';
const OUTSIDER = 'outsider-spi-9';
...
function ref(ctxDb: CtxDb, id = DOC_ID) {
  return doc(ctxDb as unknown as Parameters<typeof doc>[0], 'safety_plan_periods', id);
}
...
async function seedProjectAndDoc() {
  await requireEnv().withSecurityRulesDisabled(async (ctx) => {
    ...
    await setDoc(doc(db, 'safety_plan_periods', DOC_ID), planDoc);
  });
}

describe('safety_plan_periods — firestore.rules (server-write, member-read)', () => {
  beforeEach(async () => {
    await requireEnv().clearFirestore();
    await seedProjectAndDoc();
  });

  it('owner-allow — a project member CAN read the plan doc', async () => {
  ...
  it('integrity-deny — a member CANNOT deflate plannedInspections via update (fake >100% ratio)', async () => {
    await assertFails(updateDoc(ref(authed(MEMBER)), { plannedInspections: 1 }));
  });

  it('immutable-from-client — a member CANNOT delete the plan record', async () => {
