 ...
 *   5. NO hay function names que sugieran diagnóstico:
 *        - Prohibido: inferOccupationalDisease, assessClinicalRisk,
 *          categorizeAsLaboral.
 *        - Permitido: build*, summarize*, *ToMarkdown.
 */
...
// ─────────────────────────────────────────────────────────────────────
// Stub para wire-up Bucket VV (post-merge).
// ─────────────────────────────────────────────────────────────────────

/**
 * Stub del export que vendrá desde HealthVault (Bucket VV).
 * El orquestador post-merge lo reemplaza con el wire real que junta:
 *   - laborHistory desde PortableCurriculum
 *   - ergonomicLogs desde HealthRecord type='ergonomic_log'
 *   - symptoms desde HealthRecord type='self_reported_symptom'
 *
 * Mientras tanto este stub falla explícito para que ningún caller lo
 * confunda con una implementación real.
 */
export const exportOccupationalBundle = async (
  _uid: string,
): Promise<OccupationalContextBundle> => {
  throw new Error(
    'exportOccupationalBundle stub: Bucket VV (vaultRecord) wire pendiente post-merge',
  );
};
