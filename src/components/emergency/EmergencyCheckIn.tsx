...
export function EmergencyCheckIn() {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between p-4 bg-zinc-900/50 border border-white/10 rounded-2xl">
        <div className="flex items-center gap-3">
          <div className={`w-3 h-3 rounded-full ${isEmergencyActive ? 'bg-rose-500 animate-pulse' : 'bg-zinc-700'}`} />
      ...
      <AnimatePresence>
        {isEmergencyActive && (
          ...
          >
            {/* My Status Card */}
            <div className="p-8 bg-zinc-900 border-2 border-rose-500/50 rounded-3xl shadow-2xl shadow-rose-500/10 text-center space-y-6">
              <div className="space-y-2">
                <h3 className="text-2xl font-black text-white uppercase tracking-tighter">¿Estás a salvo?</h3>
                ...
                <button
                  ...
                  className={`flex flex-col items-center gap-3 p-6 rounded-2xl border-2 transition-all ${
                    myStatus === 'safe'
                      ...
                      : 'bg-zinc-800/50 border-white/5 text-zinc-500 hover:border-emerald-500/30 hover:text-emerald-500'
                  }`}
                >
                ...
                <button
                  ...
                  className={`flex flex-col items-center gap-3 p-6 rounded-2xl border-2 transition-all ${
                    myStatus === 'danger'
                      ...
                      : 'bg-zinc-800/50 border-white/5 text-zinc-500 hover:border-rose-500/30 hover:text-rose-500'
                  }`}
                >
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              <div className="p-4 bg-emerald-500/10 border border-emerald-500/20 rounded-2xl text-center">
                <p className="text-[10px] font-black text-emerald-500 uppercase tracking-widest mb-1">A Salvo</p>
                <p className="text-3xl font-black text-white">{stats.safe}</p>
              </div>
              <div className="p-4 bg-rose-500/10 border border-rose-500/20 rounded-2xl text-center">
                <p className="text-[10px] font-black text-rose-500 uppercase tracking-widest mb-1">En Peligro</p>
                <p className="text-3xl font-black text-white">{stats.danger}</p>
              </div>
              <div className="p-4 bg-zinc-800/50 border border-white/5 rounded-2xl text-center">
                ...
                <p className="text-3xl font-black text-white">{stats.unknown}</p>
              </div>
            </div>
                    ...
                    <div className="flex items-center gap-4">
                      <div className="text-right">
                        <p className={`text-[10px] font-black uppercase tracking-widest ${
                          worker.status === 'safe' ? 'text-emerald-500' : 
                          worker.status === 'danger' ? 'text-rose-500' : 'text-zinc-500'
                        }`}>
                          {worker.status === 'safe' ? 'A Salvo' : 
                           worker.status === 'danger' ? 'En Peligro' : 'Sin Reportar'}
                        </p>
                        <p className="text-[8px] text-zinc-600 font-bold uppercase tracking-widest">{worker.timestamp}</p>
                      </div>
                      <div className={`w-2 h-2 rounded-full ${
                        worker.status === 'safe' ? 'bg-emerald-500' : 
                        worker.status === 'danger' ? 'bg-rose-500 animate-pulse' : 'bg-zinc-700'
                      }`} />
                    </div>
                  </div>
                ))}
