...
export function industryForSector(sectorId: string): IndustryId {
  ...
  for (const [key, value] of Object.entries(GP_TO_INDUSTRY)) {
    if (
      (sectorId === key || sectorId.startsWith(`${key}-`)) &&
...
export async function defaultSubmit(
  ...
  const authorization = await apiAuthHeaderOrThrow();
  const body = JSON.stringify(payload);
  // Stable across retries/reloads, without persisting the CSV, email list or
  // auth token in browser storage. The server namespaces completion by uid.
  ...
  const res = await fetch("/api/onboarding/complete", {
    ...
    headers: {
      ...
      "Idempotency-Key": `onboarding-${key}`,
    },
    body,
  });
  if (!res.ok) {
  ...
  // Consume/validate the confirmation before the hard navigation. HTTP headers
  // alone are not a completed response (nor proof of a created project).
  const confirmed: unknown = await res.json();
  if (
    !confirmed ||
    typeof confirmed !== "object" ||
    !("success" in confirmed) ||
    confirmed.success !== true ||
    !("projectId" in confirmed) ||
    typeof confirmed.projectId !== "string" ||
    !confirmed.projectId
  ) {
    throw new Error("onboarding_confirmation_missing");
  }
}
      ...
      <div className="w-full max-w-3xl bg-slate-900 rounded-2xl shadow-2xl p-8 border border-teal-700/30">
          ...
          <IndustryStep
            ...
            onRubro={(rubro) =>
              dispatch({
                type: "SET_RUBRO",
                codigo: rubro.codigo,
                sectorId: rubro.sectorId,
              })
            }
            onWorkerCount={(count) =>
              dispatch({ type: "SET_WORKER_COUNT", count })
            }
          />
        )}
        ...
        <div className="flex justify-between mt-8">
          ...
          {isLast ? (
            ...
            >
              {state.submitting ? "Configurando…" : "Completar setup"}{" "}
              <Check size={16} />
            </button>
          ) : (
            <button
  ...
  return (
    <div>
      <h2 className="text-lg font-semibold mb-4">
        {t("onboarding.industry.title", "¿Cuál es tu industria?")}
      </h2>

      {/* SII autocomplete — picks the rubro and auto-selects the vertical. */}
      <label
        className="block text-sm text-slate-300 mb-1"
        htmlFor="sii-search-input"
      >
        {t(
          "onboarding.sii.searchLabel",
          "Busca tu rubro SII (código o actividad)",
        )}
      </label>
      <input
        id="sii-search-input"
        ...
        placeholder={t(
          "onboarding.sii.searchPlaceholder",
          "Ej: 410010 o extracción de cobre",
        )}
        ...
        >
          {results.map((r) => (
            <li key={r.codigo}>
              ...
              >
                <span className="font-mono text-teal-400 mr-2">
                  {formatCodigoSii(r.codigo)}
                </span>
                {r.descripcion}
              </button>
            </li>
      ...
      {siiCode != null && sectorId && (
        ...
        >
          ...
          <span className="block text-teal-400/80 mt-0.5">
            {t("onboarding.sii.sectorAuto", "Sector preventivo asignado")}:{" "}
            {sectorId}
          </span>
        </div>
      )}
...
function DotacionSection({
  ...
  return (
    <div className="mt-6">
      <label
        className="block text-sm text-slate-300 mb-1"
        htmlFor="workers-count-input"
      >
        {t(
          "onboarding.dotacion.label",
          "¿Cuántas personas trabajan en tu empresa? (estimado)",
        )}
      </label>
      <input
...
function CountriesStep({
  ...
  return (
    <div>
      <h2 className="text-lg font-semibold mb-4">
        {t("onboarding.countries.title", "¿En qué países operas?")}
      </h2>
...
function TierStep({
  ...
  return (
    <div>
      <h2 className="text-lg font-semibold mb-4">
        {t("onboarding.plan.title", "Elige tu plan")}
      </h2>
          ...
          return (
            ...
            >
              {popular && (
                ...
                >
                  <Sparkles size={10} />{" "}
                  {t("onboarding.plan.mostPopular", "Más popular")}
                </span>
              )}
              <div className="font-semibold">{tier.nombre}</div>
              <div className="text-2xl font-bold mt-2">
                {tier.clpRegular === 0
                  ? t("onboarding.plan.free", "Gratis")
                  : formatCurrency(tier.clpRegular, "CLP")}
                {tier.clpRegular > 0 && (
                  <span className="text-xs font-normal text-slate-400">
                    {t("onboarding.plan.perMonth", "/mes")}
                  </span>
                )}
              </div>
              <div className="text-xs text-slate-400 mt-2">
                {tier.trabajadoresMax >= 999_999
                  ? t(
                      "onboarding.plan.unlimitedWorkers",
                      "Trabajadores ilimitados",
                    )
                  : t(
                      "onboarding.plan.upToWorkers",
                      "Hasta {{count}} trabajadores",
                      { count: tier.trabajadoresMax },
                    )}
              </div>
              <div className="text-xs text-slate-400">
                {tier.proyectosMax >= 999_999
                  ? t(
                      "onboarding.plan.unlimitedProjects",
                      "Proyectos ilimitados",
                    )
                  : t(
                      "onboarding.plan.upToProjects",
                      "Hasta {{count}} proyectos",
                      { count: tier.proyectosMax },
                    )}
              </div>
            </button>
          );
  ...
  return (
    <div>
      <h2 className="text-lg font-semibold mb-2">
        {t("onboarding.team.title", "Invita a tu equipo")}
      </h2>
      <p className="text-slate-400 text-sm mb-3">
        {t(
          "onboarding.team.help",
      ...
      <textarea
        ...
        placeholder={t(
          "onboarding.team.emailsPlaceholder",
          "ana@empresa.cl, jorge@empresa.cl, …",
        )}
        rows={4}
      ...
      <div className="flex items-center justify-between mt-2 text-xs">
        ...
        <span data-testid="emails-count" className="text-slate-400">
          {parsed.length === 1
            ? t("onboarding.team.emailValidOne", "{{count}} email válido", {
                count: parsed.length,
              })
            : t(
                "onboarding.team.emailsValidOther",
                "{{count}} emails válidos",
                { count: parsed.length },
              )}
        </span>
      </div>
...
function ProjectStep({
  ...
  return (
    <div>
      <h2 className="text-lg font-semibold mb-4">
        {t("onboarding.project.title", "Tu primer proyecto")}
      </h2>
      <label className="block text-sm text-slate-300 mb-1">
        {t("onboarding.project.nameLabel", "Nombre del proyecto")}
      </label>
      <input
        type="text"
        value={name}
      ...
      <label className="block text-sm text-slate-300 mt-4 mb-1">
        {t(
          "onboarding.project.importWorkers",
      ...
      <div className="flex items-center gap-3">
        ...
        >
          <Upload size={14} />{" "}
          {workersCsv
            ? t("onboarding.project.replaceCsv", "Reemplazar CSV")
            : t("onboarding.project.uploadCsv", "Subir CSV")}
          <input
            type="file"
            accept=".csv,text/csv"
            className="hidden"
