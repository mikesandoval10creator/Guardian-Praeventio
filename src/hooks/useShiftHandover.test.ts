...
describe("useShiftHandover API client", () => {
  ...
  it("acknowledgeShiftApi → POST acknowledge with optional notes", async () => {
    fetchMock.mockResolvedValue(
      okJson({ shift: { id: "s1", acknowledged: true } }),
    );
  ...
  it("throws the server-provided error string on a non-ok response", async () => {
    ...
    await expect(
      startShiftApi("p1", { id: "s1", kind: "day" as never }),
  ...
  it("tolerates an unparseable error body (falls back to http_<status>)", async () => {
    ...
    await expect(summarizeShiftApi("p1", { shift })).rejects.toThrow(
      "http_502",
    );
  });
});
...
// ──────────────────────────────────────────────────────────────────────
// Orphan-UI stubs (rescue-450 PR #501). Ticket 39aaa66d-73fe-81ba-a153:
// `fetchShiftHandoverHistory` ahora pega al endpoint REAL
// GET /api/sprint-k/{projectId}/shift-handover/history (anti-stub).
// Los otros 3 (create/acknowledge/discrepancy) siguen pendientes de sus
// endpoints nuevos y mantienen shape pinned.
// ──────────────────────────────────────────────────────────────────────

describe("useShiftHandover — history fetch real", () => {
  it("fetchShiftHandoverHistory → GET history with days query, parses shifts", async () => {
    fetchMock.mockResolvedValue(
      okJson({
        shifts: [
          {
    ...
    expect(out.shifts[0].shift.id).toBe("s1");
    expect(out.shifts[0].quality.qualityScore).toBe(70);
    const [url, init] = fetchMock.mock.calls[0]!;
    expect(url).toBe("/api/sprint-k/p1/shift-handover/history?days=30");
    expect((init as RequestInit).method).toBe("GET");
    const headers = (init as RequestInit).headers as Record<string, string>;
    expect(headers.Authorization).toBe("Bearer test-token");
  });

  it("fetchShiftHandoverHistory defaults to days=30 when omitted", async () => {
    fetchMock.mockResolvedValue(okJson({ shifts: [] }));
    await fetchShiftHandoverHistory("p1");
    expect(fetchMock.mock.calls[0]![0]).toBe(
      "/api/sprint-k/p1/shift-handover/history?days=30",
    );
  });

  it("fetchShiftHandoverHistory throws server error string on non-ok", async () => {
    fetchMock.mockResolvedValue(errJson(403, { error: "forbidden" }));
    await expect(fetchShiftHandoverHistory("p1", { days: 7 })).rejects.toThrow(
      "forbidden",
    );
  });
});

describe("useShiftHandover orphan-UI stubs (aun pendientes de endpoints nuevos)", () => {
  ...
  it("acknowledgeShiftHandover returns a typed shell carrying the shiftId (no fetch)", async () => {
    const out = await acknowledgeShiftHandover(
      "p1",
      "s5",
      { notes: "ok" },
      "idem-key-2",
