    ...
    <div className="p-4 sm:p-6 max-w-7xl mx-auto space-y-6 sm:space-y-8">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div className="flex items-center gap-3 sm:gap-4">
          <button 
            onClick={() => navigate('/documents')}
            className="w-8 h-8 sm:w-10 sm:h-10 shrink-0 rounded-lg sm:rounded-xl bg-surface border border-default-token flex items-center justify-center text-muted-token hover:text-zinc-900 dark:hover:text-white hover:bg-zinc-100 dark:hover:bg-zinc-800 transition-colors shadow-sm"
          >
            <ArrowLeft className="w-4 h-4 sm:w-5 sm:h-5" />
          </button>
          <div className="min-w-0">
            <h1 className="text-lg sm:text-2xl font-black text-primary-token uppercase tracking-tighter truncate">{document.name || document.title}</h1>
            <p className="text-[8px] sm:text-[10px] font-bold text-muted-token uppercase tracking-widest mt-1 truncate">
      ...
      <motion.div
        ...
        className="bg-surface rounded-2xl sm:rounded-3xl p-4 sm:p-8 text-primary-token shadow-2xl overflow-hidden border border-default-token"
      >
        {document.content ? (
          typeof document.content === 'string' ? (
            ...
            <div className="space-y-6 sm:space-y-8">
              ...
              {Object.entries(document.content).map(([key, value]: [string, any]) => (
                <div key={key}>
                  <h3 className="text-sm sm:text-lg font-black uppercase tracking-widest text-primary-token mb-3 sm:mb-4 border-b border-default-token pb-2 break-words">
                    {key.replace(/([A-Z])/g, ' $1').trim()}
                  </h3>
                  {Array.isArray(value) ? (
                    <ul className="space-y-2">
                      {value.map((item, i) => (
                        <li key={i} className="flex items-start gap-2 sm:gap-3 text-xs sm:text-sm text-secondary-token">
                    ...
                    <p className="text-xs sm:text-sm text-secondary-token leading-relaxed break-words">{typeof value === 'string' ? value : JSON.stringify(value)}</p>
                  )}
                </div>
              ))}
            </div>
          ...
          <div className="w-full flex flex-col items-center justify-center">
            {document.type?.includes('image') || document.name?.match(/\.(jpg|jpeg|png|gif|webp)$/i) ? (
              ...
              <div className="text-center py-10 sm:py-20">
                ...
                <p className="text-muted-token text-xs sm:text-sm font-medium px-4">{t('documentViewer.cannotPreview', 'Este tipo de archivo no se puede previsualizar directamente.')}</p>
                <a 
                  href={document.url}
                  target="_blank"
                  rel="noopener noreferrer"
