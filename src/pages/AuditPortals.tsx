...
const AFFILIATION_LABELS: Record<AuditorAffiliation, string> = {
  mandante: 'Mandante',
  suseso: 'SUSESO',
  mutualidad: 'Mutualidad (ACHS/IST/Mutual/ISL)',
...
const MODULE_LABELS: Record<AuditModule, string> = {
  ...
  compliance_snapshot: 'Snapshot cumplimiento',
};
...
export function AuditPortals() {
  ...
  const handleCreate = useCallback(async () => {
    ...
    try {
      ...
      setFeedback(`Portal creado. Token: ${portal.accessToken.slice(0, 12)}…`);
      resetForm();
    } catch (err) {
  ...
  const handleRevoke = useCallback(
    async (portal: AuditPortalConfig) => {
      ...
      const reason = window.prompt('Motivo de la revocación (mín 5 chars):', '');
      ...
      try {
        ...
        setFeedback(`Portal ${portal.id.slice(0, 12)} revocado.`);
      } catch (err) {
        const msg = err instanceof Error ? err.message : String(err);
        setFeedback(msg);
      }
  ...
  const handleCopyToken = useCallback(async (portal: AuditPortalConfig) => {
    try {
      ...
      setFeedback('No se pudo copiar — copialo manualmente.');
    }
  }, []);
        ...
        <header className="flex items-start justify-between gap-4">
          <div>
            <h1 className="text-2xl font-black text-zinc-900 dark:text-white tracking-tight flex items-center gap-2">
              <ShieldCheck className="w-6 h-6 text-emerald-500" /> Portales de auditor
            </h1>
            <p className="text-xs text-zinc-500 mt-1 max-w-2xl">
              Acceso temporal con scope limitado para auditores externos
              (SUSESO, mutualidad, ISO certifier, mandante, SEREMI, DT).
              Token de 64 chars hex; TTL 1-90 días; módulos seleccionables.
              Revocable manualmente antes del vencimiento.
            </p>
          </div>
          <button
            type="button"
          ...
          <>
            ...
            {showForm && (
              <section className="rounded-2xl border border-emerald-200 dark:border-emerald-800 bg-emerald-50/40 dark:bg-emerald-900/15 p-4 space-y-3">
                ...
                <div className="grid grid-cols-2 gap-3">
                  <label className="space-y-1 text-xs">
                    <span className="font-bold text-zinc-700 dark:text-zinc-300">
                      Nombre auditor (mín 3 chars)
                    </span>
                    <input
                      type="text"
                      value={auditorName}
                      onChange={(e) => setAuditorName(e.target.value)}
                      placeholder="Ej: Juan Pérez Bureau Veritas"
                  ...
                  <label className="space-y-1 text-xs">
                    <span className="font-bold text-zinc-700 dark:text-zinc-300">Afiliación</span>
                    <select
                ...
                <label className="block space-y-1 text-xs">
                  <span className="font-bold text-zinc-700 dark:text-zinc-300">Email (opcional)</span>
                  <input
                    type="email"
                    value={auditorEmail}
                    onChange={(e) => setAuditorEmail(e.target.value)}
                    placeholder="auditor@institucion.cl"
                    className="w-full rounded-lg border border-zinc-300 dark:border-white/10 bg-white dark:bg-zinc-900 px-2 py-1.5 text-zinc-900 dark:text-white"
                  />
                </label>
                <label className="block space-y-1 text-xs">
                  <span className="font-bold text-zinc-700 dark:text-zinc-300">TTL en días (1-90)</span>
                  <input
                    type="number"
                    min={1}
                    max={90}
                ...
                <label className="block space-y-1 text-xs">
                  <span className="font-bold text-zinc-700 dark:text-zinc-300">
                    Notas internas (opcional, no visibles al auditor)
                  </span>
                  <textarea
                    value={internalNotes}
            ...
            <section className="space-y-2">
              <h2 className="text-xs font-black text-zinc-500 uppercase tracking-widest">
                Portales activos ({activePortals.length}) · Total ({portals.length})
              </h2>
              {portals.length === 0 ? (
                <div className="rounded-2xl border border-zinc-200 dark:border-white/10 bg-white dark:bg-zinc-900/60 p-6 text-center text-sm text-zinc-500">
                  Sin portales creados.
                </div>
              ) : (
