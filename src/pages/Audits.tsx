...
export function Audits() {
  ...
  return (
    <div className="p-4 max-w-5xl mx-auto space-y-6">
      <DataLoadErrorBanner error={nodesError} resourceLabel={t('audits.resource_label')} />
        ...
        {view === 'inspection' ? (
          ...
          >
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
              {[
                { label: t('audits.stats.total'), value: stats.total, icon: Activity, color: 'text-blue-500', bg: 'bg-blue-500/10' },
                { label: t('audits.stats.planned'), value: stats.planned, icon: Clock, color: 'text-amber-500', bg: 'bg-amber-500/10' },
                { label: t('audits.stats.completed'), value: stats.completed, icon: CheckCircle2, color: 'text-[#4db6ac] dark:text-[#d4af37]', bg: 'bg-[#4db6ac]/10' },
                { label: t('audits.stats.avg_score'), value: `${stats.avgScore}%`, icon: Target, color: 'text-indigo-500', bg: 'bg-indigo-500/10' },
              ].map((stat, i) => (
                <motion.div
                  key={stat.label}
            ...
            <div className="flex gap-2">
              ...
              >
                ...
                <span className="text-[10px] font-black uppercase tracking-widest hidden sm:inline">{t('audits.new_audit')}</span>
              </motion.button>
            </div>
            ...
            <div className="space-y-3">
              {loading ? (
                <div className="flex flex-col items-center justify-center py-12 gap-3">
                  ...
                  <p className="text-[10px] font-black uppercase tracking-widest text-zinc-400">{t('audits.syncing')}</p>
                </div>
              ) : audits.length > 0 ? (
                audits.map((audit, i) => (
                  <motion.div
