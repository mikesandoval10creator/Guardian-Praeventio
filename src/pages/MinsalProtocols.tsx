      ...
      <div className="p-6 max-w-7xl mx-auto space-y-8">
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
          <div>
            ...
            <p className="text-secondary mt-1">{t('minsalProtocols.subtitle', 'Gestión de cumplimiento normativo de salud ocupacional en Chile')}</p>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
              ...
              <motion.button
                ...
                className={`w-full text-left p-4 rounded-2xl border transition-all ${
                  selectedProtocol === protocol.id 
                    ? `bg-elevated border-${protocol.color.split('-')[1]}-500 shadow-lg`
                    : 'bg-surface border-default-token hover:bg-elevated'
                }`}
              >
                <div className="flex items-start gap-4">
                  ...
                  <div className="flex-1 min-w-0">
                    <h3 className="text-lg font-bold text-primary truncate">{protocol.title}</h3>
                    <p className="text-sm text-secondary line-clamp-2 mt-1">{protocol.description}</p>
                  </div>
          ...
          <div className="lg:col-span-2">
            <div className="bg-surface rounded-3xl border border-default-token p-6 min-h-[600px] flex flex-col">
              <AnimatePresence mode="wait">
                {!selectedProtocol ? (
                  <motion.div 
                    ...
                    animate={{ opacity: 1 }}
                    exit={{ opacity: 0 }}
                    className="flex-1 flex flex-col items-center justify-center text-muted-token space-y-4"
                  >
                  ...
                  <motion.div 
                    ...
                    animate={{ opacity: 1 }}
                    exit={{ opacity: 0 }}
                    className="flex-1 flex flex-col items-center justify-center text-secondary space-y-6"
                  >
                    ...
                    <div className="text-center space-y-2">
                      <h3 className="text-lg font-bold text-primary">Analizando Cumplimiento</h3>
                  ...
                  >
                    <div className="flex items-center gap-3 mb-6 pb-6 border-b border-default-token">
                      <div className="w-10 h-10 rounded-xl bg-emerald-500/10 flex items-center justify-center border border-emerald-500/20">
                        <CheckCircle2 className="w-5 h-5 text-emerald-500" />
                      </div>
                      <div>
                        <h2 className="text-xl font-bold text-primary">Informe de Auditoría IA</h2>
                        <p className="text-sm text-secondary">Generado basado en el contexto actual del proyecto</p>
                      </div>
                    </div>
