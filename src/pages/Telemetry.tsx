...
export function Telemetry() {
  ...
  const simulateIoTEvent = async () => {
    ...
    try {
      const context = `
        Proyecto: ${selectedProject?.name || 'Global'}
        Ubicación: ${selectedProject?.location || 'No definida'}
        Clima Actual: ${weather ? `${Math.round(weather.temp)}°C, Viento ${Math.round(weather.windSpeed)} km/h` : 'Desconocido'}
      `;
      
      const aiEvent = await generateRealisticIoTEvent(context);
      ...
      await addDoc(collection(db, 'telemetry_events'), newEvent);
    } catch (error) {
  ...
  // Auto-simulation loop for real-time feel
  useEffect(() => {
    ...
    return () => clearInterval(interval);
  }, [autoSimulate, selectedProject, weather]);
        ...
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-4">
            <h3 className="text-xs font-black uppercase tracking-widest text-white flex items-center gap-2">
              ...
              Telemetría IoT (Wearables y Maquinaria)
            </h3>
          ...
          <div className="flex items-center gap-2">
            <button
              ...
              className={`px-4 py-2 rounded-xl text-[10px] font-black uppercase tracking-widest transition-colors flex items-center gap-2 ${
                autoSimulate 
            ...
            >
              ...
              {autoSimulate ? 'Auto-Simulación Activa' : 'Iniciar Auto-Simulación'}
            </button>
            <button
              onClick={simulateIoTEvent}
              disabled={isSimulating}
              className="px-4 py-2 rounded-xl bg-zinc-800 hover:bg-zinc-700 text-white text-[10px] font-black uppercase tracking-widest transition-colors flex items-center gap-2 disabled:opacity-50 disabled:cursor-not-allowed"
            >
              {isSimulating ? (
        ...
        <div className="space-y-3">
          {iotEvents && iotEvents.length > 0 ? (
            ...
            <div className="text-center py-12 bg-zinc-950/50 rounded-2xl border border-white/5">
              ...
              <p className="text-xs text-zinc-600 mt-1">Usa el botón "Simular Evento" para generar datos de prueba.</p>
            </div>
          )}
        </div>
      </div>
