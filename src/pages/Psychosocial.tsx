  ...
  return (
    <div className="p-4 sm:p-6 lg:p-8 max-w-7xl mx-auto space-y-6 sm:space-y-8">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 sm:gap-6">
        <div>
          <h1 className="text-2xl sm:text-3xl md:text-4xl font-black text-primary-token uppercase tracking-tighter leading-tight">{t('psychosocial.title')}</h1>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 sm:gap-8">
        ...
        <div className="lg:col-span-2 space-y-4 sm:space-y-6">
          <div className="relative">
            ...
            <input
              ...
              className="w-full bg-surface border border-default-token rounded-xl sm:rounded-2xl py-3 sm:py-4 pl-10 sm:pl-12 pr-4 text-xs sm:text-sm text-primary-token placeholder:text-muted-token focus:outline-none focus:ring-2 focus:ring-rose-500/50 transition-all"
            />
          </div>
            ...
            {loading ? (
              ...
              filteredAssessments.map((node, index) => (
                <motion.div
                  ...
                  className="bg-surface border border-default-token rounded-xl sm:rounded-2xl p-4 sm:p-5 hover:border-rose-500/30 transition-all group cursor-pointer"
                >
                  <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                    <div className="flex items-start gap-3 sm:gap-4">
                      ...
                      <div>
                        <h3 className="text-sm sm:text-base font-bold text-primary-token group-hover:text-rose-400 transition-colors">
                          {node.title}
                        </h3>
                        <div className="flex flex-wrap items-center gap-2 sm:gap-3 mt-1 sm:mt-2 text-[10px] sm:text-xs text-secondary-token">
                    ...
                    <div className="flex items-center justify-between sm:justify-end gap-4 sm:w-auto w-full border-t border-default-token sm:border-t-0 pt-3 sm:pt-0">
                      <div className="text-left sm:text-right">
                        <div className="text-[10px] font-bold text-muted-token uppercase tracking-wider mb-1">{t('psychosocial.risk_level')}</div>
                      ...
                      <ChevronRight className="w-4 h-4 sm:w-5 sm:h-5 text-muted-token group-hover:text-rose-500 transition-colors" />
                    </div>
                  </div>
                </motion.div>
              ))
            ) : (
              <div className="text-center py-12 sm:py-20 bg-surface rounded-2xl border border-default-token">
                <Brain className="w-10 h-10 sm:w-12 sm:h-12 text-muted-token mx-auto mb-3 sm:mb-4" />
                <h3 className="text-sm sm:text-base font-bold text-primary-token mb-1 sm:mb-2">{t('psychosocial.empty_title')}</h3>
                <p className="text-xs sm:text-sm text-muted-token">{t('psychosocial.empty_subtitle')}</p>
              </div>
            )}
          </div>
        </div>
        ...
        <div className="space-y-4 sm:space-y-6">
          <div className="bg-surface border border-default-token rounded-2xl p-5 sm:p-6">
            <h3 className="text-[10px] sm:text-xs font-bold text-muted-token uppercase tracking-widest mb-4 sm:mb-6 flex items-center gap-2">
            ...
            <div className="space-y-4 sm:space-y-6">
              <div>
                <div className="flex justify-between text-xs sm:text-sm mb-2">
                  <span className="text-secondary-token">{t('psychosocial.coverage')}</span>
                  <span className="text-primary-token font-bold">{stats.evaluated}%</span>
                </div>
              ...
              <div className="grid grid-cols-2 gap-3 sm:gap-4">
                <div className="bg-elevated rounded-xl p-3 sm:p-4 border border-default-token">
                  <div className="text-xl sm:text-2xl font-black text-rose-500 mb-1">{stats.highRisk}</div>
                  <div className="text-[9px] sm:text-[10px] font-bold text-muted-token uppercase tracking-widest">{t('psychosocial.high_risk')}</div>
                </div>
                <div className="bg-elevated rounded-xl p-3 sm:p-4 border border-default-token">
                  <div className="text-xl sm:text-2xl font-black text-amber-500 mb-1">{stats.mediumRisk}</div>
                  <div className="text-[9px] sm:text-[10px] font-bold text-muted-token uppercase tracking-widest">{t('psychosocial.medium_risk')}</div>
                </div>
              </div>
            </div>
          </div>
          ...
          <div className="bg-gradient-to-br from-rose-500/10 to-transparent border border-rose-500/20 rounded-2xl p-5 sm:p-6">
            ...
            <p className="text-xs sm:text-sm text-secondary-token leading-relaxed">
