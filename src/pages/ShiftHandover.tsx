...
import { ShiftHandoverHistoryList } from "../components/shiftHandover/ShiftHandoverHistoryList";
...
export function ShiftHandover() {
  ...
  const CATEGORY_LABELS: Record<HandoverCategory, string> = {
    open_incidents: t(
      "shift_handover.category.open_incidents",
      "Incidentes abiertos",
    ),
    equipment_down: t("shift_handover.category.equipment_down", "Equipo fuera"),
    pending_controls: t(
      "shift_handover.category.pending_controls",
      "Controles pendientes",
    ),
    absent_workers: t("shift_handover.category.absent_workers", "Ausencias"),
    restricted_zones: t(
      "shift_handover.category.restricted_zones",
      "Zonas restringidas",
    ),
    active_permits: t(
      "shift_handover.category.active_permits",
      "Permisos activos",
    ),
  ...
  const [noteCategory, setNoteCategory] =
    useState<HandoverCategory>("observation");
  ...
  const myActiveShift = useMemo(
    () =>
      shifts.find((s) => s.supervisorUid === user?.uid && !s.endedAt) ?? null,
    [shifts, user],
  );
  ...
  const handleEnd = useCallback(async () => {
    ...
    try {
      ...
      setFeedback(
        "Turno cerrado. El siguiente supervisor puede hacer handover.",
      );
    } catch (err) {
      const msg = err instanceof Error ? err.message : String(err);
      setFeedback(msg);
  ...
  const handleAcknowledge = useCallback(async () => {
    ...
    try {
      const acked = acknowledgeHandover(
        pendingAck,
        user.uid,
        ackNotes.trim() || undefined,
      );
      ...
      <div className="max-w-5xl mx-auto px-4 sm:px-6 py-6 space-y-6">
        <header>
          <h1 className="text-2xl font-black text-primary-token tracking-tight flex items-center gap-2">
            ...
            {t("shift_handover.title", "Cambio de turno")}
          </h1>
        ...
        {!selectedProject ? (
          <div className="rounded-2xl border border-default-token bg-elevated p-6 text-center text-sm text-zinc-500">
            {t(
              "shift_handover.empty.select_project",
            ...
            {pendingAck && (
              <section className="rounded-2xl border border-rose-200 dark:border-rose-800 bg-rose-50/60 dark:bg-rose-900/20 p-4 space-y-3">
                <h2 className="text-sm font-black text-rose-700 dark:text-rose-300 uppercase tracking-widest flex items-center gap-2">
                  ...
                  {t(
                    "shift_handover.pending_ack.heading",
                    "Handover pendiente de recibir",
                  )}
                </h2>
                <p className="text-xs text-rose-700 dark:text-rose-300">
                  {t("shift_handover.pending_ack.summary", {
                    defaultValue:
                      "El supervisor saliente cerró el turno {{kind}} con {{notes}} nota(s) y {{followups}} follow-up(s). Confirmá la recepción para tomar el turno entrante.",
                    kind: SHIFT_KIND_LABELS[pendingAck.kind],
                    notes: pendingAck.handoverNotes.length,
                    followups: pendingAck.logEntries.filter(
                      (e) => e.requiresFollowUp,
                    ).length,
                  })}
                </p>
                ...
                <textarea
                  ...
                  rows={2}
                  placeholder={t(
                    "shift_handover.pending_ack.notes_placeholder",
                    "Notas opcionales del supervisor entrante (qué priorizás del handover)…",
                  )}
                ...
                >
                  ...
                  {t(
                    "shift_handover.pending_ack.confirm",
            ...
            {myActiveShift ? (
                ...
                <div className="space-y-2">
                  <h3 className="text-xs font-black text-secondary-token">
                    {t("shift_handover.log.heading", {
                      defaultValue:
                        "Agregar entrada al log ({{count}} entradas)",
                  ...
                  <input
                    ...
                    placeholder={t(
                      "shift_handover.log.placeholder",
                      "Descripción de lo que pasó (min 5 chars)…",
                    )}
                    className="w-full rounded-lg border border-default-token bg-surface px-2 py-1.5 text-xs text-primary-token"
                  />
                  <label className="flex items-center gap-2 text-[10px] text-secondary-token">
                    ...
                    />
                    {t(
                      "shift_handover.log.requires_followup",
                ...
                <div className="space-y-2 pt-3 border-t border-indigo-200 dark:border-indigo-800">
                  ...
                  <div className="grid grid-cols-2 gap-2">
                    <select
                      value={noteCategory}
                      onChange={(e) =>
                        setNoteCategory(e.target.value as HandoverCategory)
                      }
                    ...
                    <select
                      ...
                      onChange={(e) =>
                        setNoteSeverity(
                          e.target.value as "info" | "attention" | "urgent",
                        )
                      }
                      className="rounded-lg border border-default-token bg-surface px-2 py-1.5 text-xs text-primary-token"
                    >
                      <option value="info">
                        {t("shift_handover.severity.info", "Info")}
                      </option>
                      <option value="attention">
                        {t("shift_handover.severity.attention", "Atención")}
                      </option>
                      <option value="urgent">
                        {t("shift_handover.severity.urgent", "Urgente")}
                      </option>
                    </select>
                  </div>
                  <input
                    ...
                    placeholder={t(
                      "shift_handover.note.placeholder",
                      "Nota para el supervisor entrante (min 5 chars)…",
                    )}
                    className="w-full rounded-lg border border-default-token bg-surface px-2 py-1.5 text-xs text-primary-token"
                  />
                  <button
              ...
              <section className="rounded-2xl border border-default-token bg-elevated p-4 space-y-3">
                ...
                <div className="flex flex-wrap gap-2">
                  {(
                    Object.entries(SHIFT_KIND_LABELS) as Array<
                      [ShiftKind, string]
                    >
                  ).map(([k, v]) => (
                    <button
                ...
                >
                  ...
                  {t("shift_handover.cta_start", "Iniciar turno")}
                </button>
              </section>
            )}

            {/* Historial — endpoint real con quality scores (anti-stub). */}
            {selectedProject && (
