...
export function Workers() {
  ...
  const [activeModal, setActiveModal] = useState<'epp' | 'docs' | 'qr' | 'safety-plan' | 'training' | null>(null);
  ...
  return (
    <div className="p-6 max-w-7xl mx-auto">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-8">
        <div>
          <h1 className="text-3xl font-bold text-white tracking-tight">Trabajadores</h1>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
        ...
        <button className="flex items-center justify-center gap-2 bg-zinc-900/50 border border-white/10 text-zinc-400 hover:text-white hover:bg-zinc-800 rounded-xl py-2.5 transition-all">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {filteredWorkers.map((worker, index) => (
            <motion.div
              ...
              className="bg-zinc-900/50 border border-white/10 rounded-2xl p-5 hover:border-emerald-500/30 transition-all group relative overflow-hidden"
            >
              ...
              <div className="flex items-start gap-4 mb-4">
                <div className="w-16 h-16 rounded-2xl bg-zinc-800 flex items-center justify-center text-2xl font-bold text-emerald-500 border border-white/5">
                ...
                <div>
                  ...
                  <p className="text-zinc-500 text-sm font-medium">{worker.role}</p>
                  <div className="flex items-center gap-2 mt-1">
                    <span className={`w-2 h-2 rounded-full ${worker.status === 'active' ? 'bg-emerald-500' : 'bg-zinc-500'}`} />
              ...
              <div className="grid grid-cols-5 gap-1 pt-4 border-t border-white/5">
                <button 
                  ...
                  className="flex flex-col items-center gap-1 p-2 rounded-xl hover:bg-white/5 transition-colors group/btn"
                >
                  <Shield className="w-4 h-4 text-indigo-400 group-hover/btn:scale-110 transition-transform" />
                  <span className="text-[8px] uppercase font-bold text-zinc-500">EPP</span>
                </button>
                <button 
                  onClick={() => { setSelectedWorker(worker); setActiveModal('docs'); }}
                  className="flex flex-col items-center gap-1 p-2 rounded-xl hover:bg-white/5 transition-colors group/btn"
                >
                  <FileText className="w-4 h-4 text-amber-400 group-hover/btn:scale-110 transition-transform" />
                  <span className="text-[8px] uppercase font-bold text-zinc-500">Docs</span>
                </button>
                <button 
                  onClick={() => { setSelectedWorker(worker); setActiveModal('safety-plan'); }}
                  className="flex flex-col items-center gap-1 p-2 rounded-xl hover:bg-white/5 transition-colors group/btn"
                >
                  <BrainCircuit className="w-4 h-4 text-emerald-400 group-hover/btn:scale-110 transition-transform" />
                ...
                <button 
                  ...
                  className="flex flex-col items-center gap-1 p-2 rounded-xl hover:bg-white/5 transition-colors group/btn"
                >
                  <GraduationCap className="w-4 h-4 text-indigo-400 group-hover/btn:scale-110 transition-transform" />
                  <span className="text-[8px] uppercase font-bold text-zinc-500">Capac</span>
                </button>
                <button 
                  onClick={() => { setSelectedWorker(worker); setActiveModal('qr'); }}
                  className="flex flex-col items-center gap-1 p-2 rounded-xl hover:bg-white/5 transition-colors group/btn"
                >
                  <QrCode className="w-4 h-4 text-emerald-400 group-hover/btn:scale-110 transition-transform" />
                  <span className="text-[8px] uppercase font-bold text-zinc-500">QR</span>
                </button>
              </div>
            </motion.div>
          ))}
