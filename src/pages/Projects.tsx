    ...
    <div className="flex-1 w-full p-4 sm:p-6 max-w-7xl mx-auto space-y-6 sm:space-y-8">
      ...
      <PredictedActivityModal
        ...
        onSchedule={async (activity) => {
          // Map PredictedActivityType → human-readable title + calendar event type.
          const ACTIVITY_TITLE_MAP: Record<PredictedActivity['type'], string> = {
          ...
          const ACTIVITY_TYPE_MAP: Record<PredictedActivity['type'], 'Reunión' | 'Capacitación' | 'Inspección' | 'Auditoría'> = {
            'cphs-meeting': 'Reunión',
            'odi-training': 'Capacitación',
            'audiometria': 'Inspección',
            'iper-review': 'Auditoría',
            'management-review-iso45001': 'Reunión',
            'climate-risk-review': 'Inspección',
          };

          try {
            const dateStr = activity.recommendedDate.toISOString().split('T')[0];
            const timeStr = activity.recommendedDate.toLocaleTimeString('es-CL', {
              hour: '2-digit',
              minute: '2-digit',
              hour12: false,
            });
            ...
            await addDoc(
              collection(db, `projects/${activity.projectId}/calendar_events`),
              {
                title: ACTIVITY_TITLE_MAP[activity.type] ?? activity.type,
                description: activity.reason,
                date: dateStr,
                time: timeStr,
                location: '',
                type: ACTIVITY_TYPE_MAP[activity.type] ?? 'Reunión',
                projectId: activity.projectId,
                autoGenerated: false,
                priority: activity.priority,
                recommendedDurationMin: activity.recommendedDurationMin,
                legalReference: activity.legalReference ?? null,
            ...
            addNotification({
              ...
              message: `${ACTIVITY_TITLE_MAP[activity.type] ?? activity.type} programada para ${activity.recommendedDate.toLocaleDateString('es-CL')}.`,
              type: 'success',
            });
            logger.info('predicted_activity_scheduled', {
              type: activity.type,
              projectId: activity.projectId,
              recommendedDate: activity.recommendedDate.toISOString(),
            });
          } catch (err) {
            logger.error('predicted_activity_schedule_failed', {
              type: activity.type,
              projectId: activity.projectId,
              error: String(err),
            });
