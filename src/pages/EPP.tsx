...
export function EPP() {
  ...
  const categories = ['Todos', ...new Set(eppCatalog.map(item => item.category))];

  const filteredEPP = eppCatalog.filter(item => {
  ...
  return (
    <div className="p-6 max-w-7xl mx-auto space-y-8">
        ...
        <div className="flex items-center gap-3">
          <div className="bg-zinc-900/50 border border-white/10 rounded-2xl px-4 py-2 flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-sm font-bold text-white">{eppCatalog.length}</p>
            </div>
          </div>
          <button className="bg-white text-black px-6 py-3 rounded-2xl text-[10px] font-black uppercase tracking-widest hover:bg-zinc-200 transition-all shadow-xl shadow-white/5 flex items-center gap-2">
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
        {[
          { label: 'En Stock', value: '1,240', icon: CheckCircle2, color: 'text-emerald-500', bg: 'bg-emerald-500/10' },
          { label: 'Por Reponer', value: '12', icon: AlertCircle, color: 'text-amber-500', bg: 'bg-amber-500/10' },
          { label: 'Vencimientos', value: '5', icon: Clock, color: 'text-red-500', bg: 'bg-red-500/10' },
          { label: 'Asignados', value: '450', icon: Shield, color: 'text-blue-500', bg: 'bg-blue-500/10' },
        ].map((stat, i) => (
          <motion.div
            key={i}
          ...
          >
            ...
            <div className="mt-6 pt-6 border-t border-white/5 flex items-center justify-between">
              <div className="flex flex-col">
                ...
                <span className="text-sm font-bold text-white">124 u.</span>
              </div>
