...
export function EPP() {
  ...
  const {
    data: eppItems,
    loading,
    fromCache: eppFromCache,
    hasPendingWrites: eppHasPendingWrites,
  } = useFirestoreCollection<EPPItem>(
  ...
  const categories = [
    "__all__",
    ...new Set((eppItems || []).map((item) => item.category)),
  ];
  ...
  return (
    <div className="p-4 sm:p-6 lg:p-8 max-w-7xl mx-auto space-y-6 sm:space-y-8 w-full overflow-hidden box-border">
        ...
        <div className="flex flex-col sm:flex-row flex-wrap items-stretch sm:items-center gap-2 sm:gap-3">
          <div className="bg-elevated border border-zinc-200 dark:border-white/10 rounded-xl sm:rounded-2xl px-3 sm:px-4 py-2 flex items-center justify-between sm:justify-start gap-3 w-full sm:w-auto shadow-sm">
            <div className="flex items-center gap-2 sm:gap-3">
              ...
              <div>
                <p className="text-[9px] sm:text-[10px] font-black text-zinc-500 uppercase tracking-widest leading-tight">
                  {t("epp.total_items")}
                </p>
                <p className="text-xs sm:text-sm font-bold text-primary-token mt-0.5">
                  {eppItems?.length || 0}
                </p>
              </div>
            </div>
      ...
      {eppFromCache && (
        <div
          role="status"
          data-testid="epp-cache-stale-warning"
          className="rounded-2xl border border-amber-300 bg-amber-50 p-3 text-xs font-bold text-amber-800 dark:border-amber-700 dark:bg-amber-900/30 dark:text-amber-100"
        >
          Catálogo EPP posiblemente desactualizado: Firestore lo está sirviendo
          desde caché local por señal baja o sin conexión. Verifica stock/EPP
          vigente antes de tomar decisiones críticas.
        </div>
      )}
      ...
      {eppHasPendingWrites && (
        <div
          role="status"
          data-testid="epp-pending-writes-warning"
          className="rounded-2xl border border-sky-300 bg-sky-50 p-3 text-xs font-bold text-sky-800 dark:border-sky-700 dark:bg-sky-900/30 dark:text-sky-100"
        >
          Hay cambios EPP pendientes de sincronizar con Firestore.
        </div>
      )}
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3 sm:gap-4">
        {[
          ...
          {
            label: t("epp.expirations"),
            value: (eppAssignments || [])
              .filter(
                (a) =>
                  a.status === "active" &&
                  a.expiresAt &&
          ...
          >
            <div
              className={`w-10 h-10 sm:w-12 sm:h-12 rounded-xl sm:rounded-2xl ${stat.bg} flex items-center justify-center border border-white/5 shrink-0`}
            >
            ...
            <div>
              ...
              <p className="text-lg sm:text-xl font-black text-primary-token tracking-tight mt-0.5">
                {stat.value}
              </p>
            </div>
          </motion.div>
              ...
              onSubmitted={(result) => {
                setInspectionMsg(
                  result.suggestedOrder
                    ? t(
                        "epp.inspect_submitted_with_order",
                        "Inspección registrada. Se sugirió una orden de compra para revisión.",
                      )
                    : t(
                        "epp.inspect_submitted",
        ...
        {filteredEPP.map((item, index) => (
          ...
          >
            <div className="absolute top-0 right-0 p-4 sm:p-6">
              <div
                className={`w-2 h-2 rounded-full ${item.required ? "bg-emerald-500 shadow-[0_0_10px_rgba(16,185,129,0.5)]" : "bg-zinc-300 dark:bg-zinc-700"}`}
              />
            </div>
            ...
            <div className="space-y-2 sm:space-y-3 flex-1">
              <div className="flex items-center gap-2 flex-wrap">
                ...
                {eppFromCache && (
                  <span
                    data-testid={`epp-cache-stale-item:${item.id}`}
                    className="inline-flex items-center gap-1 rounded-md border border-amber-200 bg-amber-50 px-2 py-1 text-[9px] font-black uppercase tracking-widest text-amber-700 dark:border-amber-700 dark:bg-amber-900/30 dark:text-amber-200"
                  >
            ...
            <div className="mt-4 sm:mt-6 pt-4 sm:pt-6 border-t border-zinc-200 dark:border-white/5 flex items-center justify-between">
              <div className="flex flex-col">
                <span className="text-[9px] sm:text-[10px] font-black text-zinc-500 dark:text-zinc-600 uppercase tracking-widest">
                  {t("epp.stock_available")}
                </span>
                <span className="text-xs sm:text-sm font-bold text-primary-token mt-0.5">
                  {item.stock || 0} {t("epp.units_short")}
                </span>
              </div>
      ...
      {filteredEPP.length === 0 && !loading && (
        <div className="text-center py-12 sm:py-20 bg-zinc-50 dark:bg-zinc-900/20 rounded-3xl sm:rounded-[3rem] border border-dashed border-zinc-200 dark:border-white/5 px-4">
          ...
          <h3 className="text-lg sm:text-xl font-black text-primary-token uppercase tracking-tight">
            {t("epp.empty_title")}
          </h3>
          <p className="text-zinc-500 text-xs sm:text-sm mt-2 uppercase tracking-widest font-bold">
            {t("epp.empty_subtitle")}
          </p>
        </div>
      )}
        ...
        {isAdding && (
          ...
          >
            ...
            >
              ...
              <h2 className="text-2xl font-black text-primary-token uppercase tracking-tight mb-6">
                {t("epp.add_modal_title")}
              </h2>

              <form onSubmit={handleAddItem} className="space-y-4">
                <div>
                  <label className="block text-xs font-bold text-muted-token uppercase tracking-widest mb-2">
                    {t("epp.field_name")}
                  </label>
                  <input
                    type="text"
