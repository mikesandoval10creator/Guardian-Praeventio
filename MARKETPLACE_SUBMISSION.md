...
2. **Stand up `contacto@praeventio.net` mailbox** (Google Workspace mail or external IMAP). Per the 2026-05-17 directive there is exactly ONE company contact address; route reviewer / support / privacy / sales mail through internal labels on that single inbox instead of separate addresses.
...
9. **Once approved:** subscribe at least one human to Marketplace email notifications for the listing (`dahosandoval@gmail.com` already covered as developer contact, plus `contacto@praeventio.net` for review traffic).
